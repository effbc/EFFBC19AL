pageextension 70067 GLAccountCardExt extends "G/L Account Card"
{
    layout
    {
        ...
        addafter("Default Deferral Template Code")
        {
            ...
            {
                Caption = 'P&L Setups';
                field("Reflect in P&L"; "Reflect in P&L")
                {
                    ...
                    trigger OnValidate();
                    begin
                        IF ("Reflect in P&L" = "Reflect in P&L"::Yes) THEN BEGIN
                            Rec."PL Head" := TRUE;
                            ...
                            Rec.MODIFY;
                        END;
                        IF ("Reflect in P&L" = "Reflect in P&L"::No) THEN BEGIN
                            Rec."PL Head" := FALSE;
                            Rec.Reflected_in_pl_datetime := CURRENTDATETIME;
                            Rec.Reflected_in_pl_userid := USERID;
    ...
    begin
        ...
        IF Rec."PL Head" = TRUE THEN BEGIN
            "Reflect in P&L" := "Reflect in P&L"::Yes;
            Rec."Reflect in P&L" := Rec."Reflect in P&L"::Yes;
            Rec.MODIFY;
        END;
