            ...
            field("Daily Entrires Posting Date"; "Daily Entrires Posting Date")
            {
                ...
                begin
                    ...
                    */
                    ...
                    lastdate := CALCDATE('CM', "Daily Entrires Posting Date");
                    ...
                    IF "Daily Entrires Posting Date" = lastsat THEN BEGIN
                        stockstatementmail;
                    END;
                    //end by pranavi
    ...
    */
    ...
    begin
        ...
        IF Search_String = 'OPEN' THEN BEGIN
            ...
            Shortage_Details.SETFILTER(Shortage_Details."Planned Purchase Date", '%1|%2..%3', 0D, "Shortage. Calc. Date",
                                                                                               ("Shortage. Calc. Date" + 2));
    ...
    begin
        ...
        IF EVALUATE(PODateVar, '31-03-2015') = TRUE THEN BEGIN
            ...
            SMTP_MAIL.AppendBody('<body><div style="border-color:#666699;  margin: 20px; border-width:15px;   border-style:solid; padding: 20px; width: 600px;"><label><font size="6">Pending Purchase Orders</font></label>');
            SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
            SMTP_MAIL.AppendBody('<h>Dear Purchase Dept. ,</h><br><br>');
            SMTP_MAIL.AppendBody('<h><b>Responsible Dept: <font color=red>Purchase.</font></h><br>');
            SMTP_MAIL.AppendBody('<h>Action: <font color=red>Need to forward PO to vendors.</font></b></h><br>');
            SMTP_MAIL.AppendBody('<P> The below Purchase Orders have been created and order is pending, </P>');
            SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:100%; font-size:10pt;"><tr><th >Order No.</th><th>Vendor</th><th>Order Date</th><th>Pending Days</th></tr>');

            IF PurchaseHeader.FINDSET THEN
                REPEAT
                    IF (TODAY - PurchaseHeader."Order Date") > 2 THEN BEGIN
                        SMTP_MAIL.AppendBody('<tr><td>' + PurchaseHeader."No." + '</td><td>' + PurchaseHeader."Buy-from Vendor Name" + '</td><td>' + FORMAT(PurchaseHeader."Order Date") + '</td><td>' + FORMAT(TODAY - PurchaseHeader."Order Date") + '</td></tr>');
                        Mail_count += 1;
                    END;
                UNTIL PurchaseHeader.NEXT = 0;

            SMTP_MAIL.AppendBody('</table><br>');
            SMTP_MAIL.AppendBody('<br>Click <a href="http://erpserver:8080/SSRSReports/Pages/ReportViewer.aspx?%2fPurchase_order_details%2fPur_Pending_Mail&rs:Command=Render"><b>here</b></a> to view the Pending Purchase Orders Report');
            SMTP_MAIL.AppendBody('<p align ="left"> Regards,<br>ERP Team </p>');
            SMTP_MAIL.AppendBody('<br><p align = "center">:: Note: Auto Generated mail from ERP if pending days greater than <b>2 days</b> :: </b></P></div></body></html>');
    ...
    begin
        ...
        Body := '';
        SMTP_MAIL.CreateMessage('ERP', Mail_From, Mail_To, Subject, Body, TRUE);
        SMTP_MAIL.AppendBody('<html><head><style> divone{background-color: white; width: 700px; padding: 20px; border-style:solid ; border-color:#FF8000;  margin: 20px;} </style></head>');
        SMTP_MAIL.AppendBody('<body><div style="border-color:#FF8000;  margin: 20px; border-width:15px;   border-style:solid; padding: 20px; width: 600px;"><label><font size="6">QA Inward Pending Items</font></label>');
        SMTP_MAIL.AppendBody('<hr style=solid; color= #FF8000>');
        SMTP_MAIL.AppendBody('<h>Dear Sir/Madam ,</h><br><br>');
        SMTP_MAIL.AppendBody('<P> The below table represents the pending Items at the QA Inward.</P>');
        SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:60%; font-size:12pt;"><tr><th align="left"><font color="brown">Period</font></th><th>2-5 Days</th><th>6-10 Days</th><th>>10 Days</th></tr>');
        SMTP_MAIL.AppendBody('<tr><td><b><font color="brown">Pending Items</font></b></td><td align="right">' + FORMAT(Pending_QA5) + '</td><td align="right">' + FORMAT(Pending_QA10) + '</td><td align="right">' + FORMAT(Pending_QAG) + '</td></tr>');
        SMTP_MAIL.AppendBody('</table><br>');
        SMTP_MAIL.AppendBody('<br>Click <a href="http://intranet:8080/QCReport/QASReport.aspx"><b>here</b></a> to view the QA Report');
        SMTP_MAIL.AppendBody('<p align ="left"> Regards,<br>ERP Team </p>');
    ...
    begin
        ...
        Body := '';
        SMTP_MAIL.CreateMessage('ERP', Mail_From, Mail_To, Subject, Body, TRUE);
        SMTP_MAIL.AppendBody('<html><head><style> divone{background-color: white; width: 700px; padding: 20px; border-style:solid ; border-color:#8EB52B;  margin: 20px;} </style></head>');
        SMTP_MAIL.AppendBody('<body><div style="border-color:#8EB52B;  margin: 20px; border-width:15px;   border-style:solid; padding: 20px; width: 600px;"><label><font size="6">To be Received Bills List</font></label>');
        SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
        SMTP_MAIL.AppendBody('<h>Dear Purchase Dept. ,</h><br><br>');
        SMTP_MAIL.AppendBody('<h><b>Responsible Dept: <font color=red>Purchase.</font></h><br>');
        SMTP_MAIL.AppendBody('<h>Action: <font color=red>Need to Receive Bill from vendors.</font></b></h><br>');
        SMTP_MAIL.AppendBody('<P> For the below Purchase Orders Material Received but Bill need to be Received , </P>');
        SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:100%; font-size:10pt;"><tr><th >Vendor</th><th>Receipt No.</th><th>Order No.</th><th>Pending Days</th></tr>');

        IF PurchRcptHeader.FINDSET THEN
            REPEAT
            BEGIN
                SMTP_MAIL.AppendBody('<tr><td>' + PurchRcptHeader."Buy-from Vendor Name" + '</td><td>' + PurchRcptHeader."No." + '</td><td>' + PurchRcptHeader."Order No." + '</td><td>' + FORMAT(TODAY - PurchRcptHeader."Posting Date") + '</td></tr>');
                Mail_count += 1;
            END;
            UNTIL PurchRcptHeader.NEXT = 0;

        SMTP_MAIL.AppendBody('</table><br>');
        SMTP_MAIL.AppendBody('<br>Click <a href="http://erpserver:8080/SSRSReports/Pages/ReportViewer.aspx?%2fPurchase_Links%2fSTR_BillsNotRcvd_Inwards&rs:Command=Render"><b>here</b></a> to view the Bills Not Received From Vendors Report');
        SMTP_MAIL.AppendBody('<p align ="left"> Regards,<br>ERP Team </p>');
        SMTP_MAIL.AppendBody('<br><p align = "center">:: Note: Auto Generated mail from ERP for Pending bills from last <b>7 Days</b> :: </P></div></body></html>');
        SMTP_MAIL.AddCC('erp@efftronics.com');
    ...
    begin
        ...
        */
        ...
        Mail_To := 'erp@efftronics.com,sujani@efftronics.com';
        Subject := 'Reg: ERP - Transaction Details as on ' + FORMAT(FromDate);       //need to update every year
        SMTP_MAIL.CreateMessage('ERP', Mail_From, Mail_To, Subject, Body, TRUE);
        SMTP_MAIL.AppendBody('<html><head><style> divone{background-color: white; width: 900px; padding: 20px; border-style:solid ; border-color:#8EB52B;  margin: 20px;} </style></head>');
        SMTP_MAIL.AppendBody('<body><div style="border-color:#8EB52B;  margin: 20px; border-width:15px;   border-style:solid; padding: 20px; width: 800px;"><label><font size="5">Consolidate Sales & AMC Transactions Summary of 18-19 Year</font></label>');
        SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
        SMTP_MAIL.AppendBody('<h3><center>' + FORMAT(FromDate, 0, '<Day>-<Month Text,3>-<Year4>') + ' Transactions');
        ...
        SMTP_MAIL.AppendBody('<tr><td>Sales</td><td align="right">' + S1 + '</td></tr>');
        SMTP_MAIL.AppendBody('<tr><td>Received</td><td align="right">' + S2 + '</td></tr>');
        SMTP_MAIL.AppendBody('<tr><td>Paid</td><td align="right">' + S3 + '</td></tr></table>');
        SMTP_MAIL.AppendBody('<h3><center>Sales Transactions(Amount in Crores) as on ' + FORMAT(FromDate, 0, '<Day>-<Month Text,3>-<Year4>'));
        SMTP_MAIL.AppendBody('</center></h3>');
        SMTP_MAIL.AppendBody('<table style="WIDTH:400px; HEIGHT: 10px; FONT-WEIGHT: bold"');
        SMTP_MAIL.AppendBody('border="1" align="Center">');
        SMTP_MAIL.AppendBody('<tr><td>Excise Sales</td><td align="right">' + FORMAT(ROUND(actsales)) + '</td></tr>');
        SMTP_MAIL.AppendBody('<tr><td>Trading Items Sales</td><td align="right">' + FORMAT(ROUND(boi)) + '</td></tr>');
        SMTP_MAIL.AppendBody('<tr><td bgcolor="#CCFFCC">Sales Turnover</td><td bgcolor="#CCFFCC" align="right">' + FORMAT(ROUND(boi + actsales)));
        SMTP_MAIL.AppendBody('</td></tr><tr><td>Installation</td><td align="right">' + FORMAT(ROUND(installationamts)) + '</td></tr>');//
        SMTP_MAIL.AppendBody('<tr><td>AMC</td><td align="right">' + FORMAT(ROUND(amc)) + '</td></tr>');
        SMTP_MAIL.AppendBody('<tr><td bgcolor="#CCFFCC">Installa&AMC Totals</td><td bgcolor="#CCFFCC" align="right">');
        SMTP_MAIL.AppendBody(FORMAT(ROUND(amc + installationamts)) + '</td></tr>');
        SMTP_MAIL.AppendBody('<tr><td bgcolor="#F0B4BC">Total</td><td bgcolor="#F0B4BC" align="right">' + FORMAT(ROUND(totalsales1)) + '</td></tr>');
        SMTP_MAIL.AppendBody('</table><br><br>');
        ...
        IF (AFAmt > 0) OR (AFlAmt > 0) OR (CSAFAmt > 0) OR (CSAFlAmt > 0) OR (EmpAmt > 0) OR (LnAmt > 0) OR (PAAmt > 0) OR (PBAmt > 0) OR (PCODAmt > 0) OR (TaxAmt > 0) THEN BEGIN
            SMTP_MAIL.AppendBody('<br><label><font size="5">Cashflow Authorized Amount Details</font></label>');
            SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
            SMTP_MAIL.AppendBody('<P> CashFlow Authorized Amounts on <b>' + FORMAT(FromDate) + ': </b></P>');
            SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:100%; font-size:10pt;"><tr><th >Head Type</th><th>Person</th><th>Reason</th><th>In Favour Of</th>');
            SMTP_MAIL.AppendBody('<th>Authorized Person</th><th>Amount</th></tr>');
            WHILE NOT RecordSet.EOF DO BEGIN
                IF (PrevRec <> FORMAT(RecordSet.Fields.Item('type').Value)) THEN BEGIN
                    IF PrevRec <> '' THEN BEGIN
                        ...
                        SMTP_MAIL.AppendBody('<tr><td colspan="5" align="center" style="color:#FF0000"><b>' + PrevRec + ' Total </b></td><td align="right" style="color:#FF0000"><b>' + FORMAT(SubTot) + '</b></td></tr>');
                    END;  //IF_PrvRec<>'' end
                    CASE FORMAT(RecordSet.Fields.Item('TYPE').Value) OF
                        ...
                        Tax:
                            ...
                            END;
                    END;
                    SMTP_MAIL.AppendBody('<tr><td rowspan = "' + FORMAT(RowSpn) + '">' + FORMAT(RecordSet.Fields.Item('type').Value) + '</td><td>' + FORMAT(RecordSet.Fields.Item('person').Value) + '</td><td>' + FORMAT(RecordSet.Fields.Item('reason').Value) + '</td>');
                    SMTP_MAIL.AppendBody('<td>' + FORMAT(RecordSet.Fields.Item('infavourof').Value) + '</td><td>' + FORMAT(RecordSet.Fields.Item('authpersonname').Value) + '</td><td align ="right">' + FORMAT(RecordSet.Fields.Item('amount1').Value) + '</td></tr>');
                END
                ELSE BEGIN
                    SMTP_MAIL.AppendBody('<tr><td>' + FORMAT(RecordSet.Fields.Item('person').Value) + '</td><td>' + FORMAT(RecordSet.Fields.Item('reason').Value) + '</td>');
                    SMTP_MAIL.AppendBody('<td>' + FORMAT(RecordSet.Fields.Item('infavourof').Value) + '</td><td>' + FORMAT(RecordSet.Fields.Item('authpersonname').Value) + '</td><td align ="right">' + FORMAT(RecordSet.Fields.Item('amount1').Value) + '</td></tr>');
                END;
                DecimalConv := 0;
                TempAmtText := '';
            ...
            END;  //CASE_PREVREC END
            SMTP_MAIL.AppendBody('<tr><td colspan="5" align="center" style="color:#FF0000"><b>' + PrevRec + ' Total </b></td><td align="right" style="color:#FF0000"><b>' + FORMAT(SubTot) + '</b></td></tr>');
            SMTP_MAIL.AppendBody('<tr><td colspan="5" align="center" style="color:#FF0000"><b> Day Total Authorized Amount  </b></td><td align="right" style="color:#FF0000"><b>' + FORMAT(TotalAmt1) + '</b></td></tr>');
            SMTP_MAIL.AppendBody('</table><br>');
            SMTP_MAIL.AppendBody('<br>Click <a href="http://erpserver:8080/ReportMGR/Pages/Report.aspx?ItemPath=%2fCashFlow+Authorisation+Report%2fAuthorisations"><b>here</b></a> to view the detailed report for about <b>1 Month</b>');
        END;
        // Added by Rakesh on 24-Nov-14 for automail when Shoratge is not run for more than 1 week
        ...
        IF NOT (RecordSet.EOF OR RecordSet.BOF) THEN BEGIN
            ...
            IF (TODAY - DT2DATE("Auth Date")) > 4 THEN         // Modified for 3 days
            BEGIN
                SMTP_MAIL.AppendBody('<br><br><label><font size="5">Delay in Shoratge Procurement Process</font></label>');
                SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
                SMTP_MAIL.AppendBody('<P>There has been a delay about more 3 days of Shortage Material Procurement process.<br>The last Shortage was executed on ' + FORMAT("Auth Date") + '.<br></P>');
                SMTP_MAIL.AppendBody('So Kindly execute the Shortage process.<br>');
            END;
        ...
        IF ((Pending_QA5 > 0) OR (Pending_QA10 > 0) OR (Pending_QAG > 0)) THEN BEGIN
            SMTP_MAIL.AppendBody('<br><br><label><font size="5">QA Inward Pending Items</font></label>');
            SMTP_MAIL.AppendBody('<hr style=solid; color= ##3333CC>');
            SMTP_MAIL.AppendBody('<P> The below table represents the pending Items at the QA Inward.</P>');
            SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:60%; font-size:12pt;"><tr><th align="left"><font color="brown">Period</font></th><th>2-5 Days</th><th>6-10 Days</th><th>>10 Days</th></tr>');
            SMTP_MAIL.AppendBody('<tr><td><b><font color="brown">Pending Items</font></b></td><td align="right">' + FORMAT(Pending_QA5) + '</td><td align="right">' + FORMAT(Pending_QA10) + '</td><td align="right">' + FORMAT(Pending_QAG) + '</td></tr>');
        ...
        IF EVALUATE(PODateVar, '31-03-2015') = TRUE THEN BEGIN
            ...
            IF Mail_count > 0 THEN BEGIN
                SMTP_MAIL.AppendBody('<br><br><br><label><font size="5">Pending Purchase Orders</font></label>');
                SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
                SMTP_MAIL.AppendBody('<h><b>Responsible Dept: <font color=red>Purchase.</font></h><br>');
                SMTP_MAIL.AppendBody('<h>Action: <font color=red>Need to forward PO to vendors.</font></b></h><br>');
                SMTP_MAIL.AppendBody('<P> The below Purchase Orders have been created and order is pending, </P>');
                ...
                IF PurchaseHeader.FINDSET THEN
                    REPEAT
                        IF (TODAY - PurchaseHeader."Order Date") > 2 THEN BEGIN
                            SMTP_MAIL.AppendBody('<tr><td>' + PurchaseHeader."No." + '</td><td>' + PurchaseHeader."Buy-from Vendor Name" + '</td><td>' + FORMAT(PurchaseHeader."Order Date") + '</td><td align="center">' + FORMAT(TODAY - PurchaseHeader."Order Date") + '</td></tr>');
                        END;
                    UNTIL PurchaseHeader.NEXT = 0;
                SMTP_MAIL.AppendBody('</table><br>');
                SMTP_MAIL.AppendBody('<br>Click <a href="http://erpserver:8080/SSRSReports/Pages/ReportViewer.aspx?%2fPurchase_order_details%2fPur_Pending_Mail&rs:Command=Render"><b>here</b></a> to view the Pending Purchase Orders Report');
        ...
        IF recCount > 0 THEN BEGIN
            SMTP_MAIL.AppendBody('<br><br><br><label><font size="5">Billing Deviated AMC Details</font></label>');
            SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
            SMTP_MAIL.AppendBody('<P> The below are details of To Be Shipped AMCs, </P>');
            SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:100%; font-size:10pt;"><tr><th >AMC order No.</th><th >Customer</th>');
            SMTP_MAIL.AppendBody('<th>Line No.</th><th>Item No.</th><th>Description</th><th>Shipement Date</th><th>Deviated Days</th><th>Amount</th></tr>');
            recCount := 0;
            SL.RESET;
            SL.SETCURRENTKEY("Document Type", "Document No.", "Line No.");
            SL.SETFILTER(SL."Document Type", '%1', SL."Document Type"::Amc);
            ...
            IF SL.FINDSET THEN
                REPEAT
                    ...
                    SMTP_MAIL.AppendBody('<tr><td>' + SL."Document No." + '</td><td>' + Cust.Name + '</td><td align = "right">' + FORMAT(SL."Line No.") + '</td><td align = "center">' + SL."No." + '</td>');
                    SMTP_MAIL.AppendBody('<td>' + SL.Description + '</td><td align = "center">' + FORMAT(SL."Shipment Date", 0, '<Day,2>-<Month Text,3>-<Year>') + '</td><td align = "right">' + FORMAT(TODAY() - SL."Shipment Date"));
                    SMTP_MAIL.AppendBody('</td><td align = "right">' + formataddress.ChangeCurrency(ROUND(UnitPrice)) + '</td></tr>');
                    recCount := recCount + 1;
                    TotalPrice += SL."Unit Price";
                UNTIL SL.NEXT = 0;
            SMTP_MAIL.AppendBody('<tr><td colspan="7" align="center" style="color:#FF0000"><b>Total Amount</b></td><td align="right" style="color:#FF0000"><b>' + formataddress.ChangeCurrency(ROUND(TotalPrice)) + '</b></td></tr>');
            SMTP_MAIL.AppendBody('</table><br>');
            SMTP_MAIL.AppendBody('<p align ="left">Note:: Please clear the above orders</p>');
        END;
        //End by Pranavi
        SMTP_MAIL.AppendBody('<p align ="left"> Regards,<br>ERP Team </p>');
    ...
    begin
        ...
        LMD_Count := 0;
        Mail_From := 'erp@efftronics.com';
        Mail_To := 'controlroom@efftronics.com,padmaja@efftronics.com,sales@efftronics.com,anilkumar@efftronics.com,erp@efftronics.com';
        //Mail_To := 'pranavi@efftronics.com';
        headerdate := CalMngmt.CalcDateBOC('-1D', TODAY(), 3, 'PROD', '', 3, 'PROD', '', FALSE);  //Added by pranavi on 27-jan-2016 to consider public holidays also
        /*
        ...
        */ //commented by pranavi on 27-jan-2016 to consider public holidays also
        ...
        Body := '';
        SMTP_MAIL.CreateMessage('ERP', Mail_From, Mail_To, Subject, Body, TRUE);
        SMTP_MAIL.AppendBody('<html><head><style> divone{background-color: white; width: 1200px; padding: 20px; border-style:solid ; border-color:#666699;  margin: 20px;} </style></head>');
        SMTP_MAIL.AppendBody('<body><div style="border-color:#666699;  margin: 20px; border-width:15px;   border-style:solid; padding: 20px; width: 1100px;"><label><font size="6">Sale Order Changes Details</font></label>');
        SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
        SMTP_MAIL.AppendBody('<h>Dear Sir/Madam,</h><br><br>');
        //SMTP_MAIL.AppendBody('<h><b>Responsible Dept: <font color=red>Manufacturing.</b></font></h><br>');
        SMTP_MAIL.AppendBody('<P> The below are details of Changes made in Sale Order on ' + FORMAT(headerdate) + ', </P>');
        //SMTP_MAIL.AppendBody('<P> Order Modifications, </P>');
        //SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:100%; font-size:10pt;"><tr><th >Sale order No.</th><th >Customer</th><th>Line No.</th><th>Item No.</th><th>Description</th><th>Type of Change</th>');
        //SMTP_MAIL.AppendBody('<th>Changed Field</th><th>Previous Value</th><th>New Value</th><th>User ID</th><th>Time of Change</th></tr>');
        /*
        IF DATE2DWY(TODAY(),1)<>1 THEN
        ...
        */ //commented by pranavi on 27-jan-2016 to consider public holidays also
                ...
                IF SH.FINDFIRST THEN BEGIN      // If Sales Header Rec Find
                    ...
                    IF releasedflag = TRUE THEN BEGIN   // If order released atleast onces
                        ...
                        IF modifyflag1 = FALSE THEN BEGIN
                            SMTP_MAIL.AppendBody('<P> Order Modifications, </P>');
                            SMTP_MAIL.AppendBody('<P> Call Letter Status Modifications, </P>');
                            SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:100%; font-size:10pt;"><tr><th >Sale order No.</th><th >Customer</th><th>Type of Change</th>');
                            SMTP_MAIL.AppendBody('<th>Previous Value</th><th>New Value</th><th>User ID</th><th>Time of Change</th></tr>');
                            modifyflag1 := TRUE;
                        END;
                        IF (COPYSTR(CHLG."Primary Key Field 2 Value", 14, 2) = '/L') OR (COPYSTR(CHLG."Primary Key Field 2 Value", 14, 2) = '/T') THEN
                            LMD_Count := 1;
                        SMTP_MAIL.AppendBody('<tr><td>' + CHLG."Primary Key Field 2 Value" + '</td><td>' + custname + '</td><td>' + 'Call Letter Status Change' + '</td>');
                        SMTP_MAIL.AppendBody('<td>' + OldCLStusVal + '</td><td>' + NewCLStusVal + '</td><td>' + username + '</td><td>' + FORMAT(CHLG."Date and Time") + '</td></tr>');
                        recCount := recCount + 1;
                    END;
                END;
            UNTIL CHLG.NEXT = 0;
        ...
        IF CHLG.FINDSET THEN
                                ...
                                IF (CHLG."Type of Change" = 1) AND (CHLG."Old Value" <> '0') THEN BEGIN
                                    IF modifyflag = FALSE THEN BEGIN
                                        IF modifyflag1 = FALSE THEN
                                            SMTP_MAIL.AppendBody('<P> Order Modifications, </P>');
                                        SMTP_MAIL.AppendBody('<P>Changes in Sales/Schedule Lines, </P>');
                                        SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:100%; font-size:10pt;"><tr><th >Sale order No.</th><th >Customer</th><th>Line No.</th><th>Schdle Line No.</th><th>Item No.</th>');
                                        SMTP_MAIL.AppendBody('<th>Description</th><th>Type of Change</th><th>Changed Field</th><th>Previous Value</th><th>New Value</th><th>User ID</th><th>Time of Change</th></tr>');
                                        modifyflag := TRUE;
                                    END;
                                    TypeOfChange := 'Quantity Change';
                                    IF (COPYSTR(CHLG."Primary Key Field 2 Value", 14, 2) = '/L') OR (COPYSTR(CHLG."Primary Key Field 2 Value", 14, 2) = '/T') THEN
                                        LMD_Count := 1;
                                    SMTP_MAIL.AppendBody('<tr><td>' + CHLG."Primary Key Field 2 Value" + '</td><td>' + custname + '</td><td>' + CHLG."Primary Key Field 3 Value" + '</td><td>' + ' ' + '</td><td>' + itemNo + '</td><td>' + Desc + '</td>');
                                    SMTP_MAIL.AppendBody('<td>' + TypeOfChange + '</td><td>Quantity</td><td>' + CHLG."Old Value" + '</td><td>' + CHLG."New Value" + '</td><td>' + username + '</td><td>' + FORMAT(CHLG."Date and Time") + '</td></tr>');
                                    recCount := recCount + 1;
                                END
                            ...
                            ELSE
                                IF (CHLG."Field No." = 60) AND (CHLG."Table No." = 37) THEN BEGIN // If field is qty shipped in SL
                                    IF (CHLG."Type of Change" = 1) THEN BEGIN
                                        ...
                                        SMTP_MAIL.AppendBody('<tr><td>' + CHLG."Primary Key Field 2 Value" + '</td><td>' + custname + '</td><td>' + CHLG."Primary Key Field 3 Value" + '</td><td>' + ' ' + '</td><td>' + itemNo + '</td><td>' + Desc + '</td>');
                                        SMTP_MAIL.AppendBody('<td>' + TypeOfChange + '</td><td>Qty Shipped</td><td>' + CHLG."Old Value" + '</td><td>' + CHLG."New Value" + '</td><td>' + username + '</td><td>' + FORMAT(CHLG."Date and Time") + '</td></tr>');
                                        recCount := recCount + 1;
                                    END
                                ...
                                ELSE
                                    IF (CHLG."Field No." = 7) AND (CHLG."Table No." = 60095) THEN BEGIN // If field is Schedule qty
                                        IF (CHLG."Type of Change" = 1) AND (CHLG."Old Value" <> '0') THEN BEGIN
                                            ...
                                            SMTP_MAIL.AppendBody('<tr><td>' + CHLG."Primary Key Field 2 Value" + '</td><td>' + custname + '</td><td>' + CHLG."Primary Key Field 3 Value" + '</td><td>' + SchLine + '</td><td>' + itemNo + '</td><td>' + Desc + '</td>');
                                            SMTP_MAIL.AppendBody('<td>' + TypeOfChange + '</td><td>Quantity</td><td>' + CHLG."Old Value" + '</td><td>' + CHLG."New Value" + '</td><td>' + username + '</td><td>' + FORMAT(CHLG."Date and Time") + '</td></tr>');
                                            recCount := recCount + 1;
                                        END
                                            ...
                                            IF (CHLG."Field No." = 6) AND (CHLG."Table No." = 37) THEN BEGIN // If field item no.
                                                ...
                                                IF TypeOfChange <> 'New Item Insertion' THEN BEGIN   // if change is Insertion
                                                    IF CHLG."Old Value" <> '' THEN BEGIN
                                                        SMTP_MAIL.AppendBody('<tr><td>' + CHLG."Primary Key Field 2 Value" + '</td><td>' + custname + '</td><td>' + CHLG."Primary Key Field 3 Value" + '</td><td>' + ' ' + '</td><td>' + itemNo + '</td><td>' + Desc + '</td>');
                                                        SMTP_MAIL.AppendBody('<td>' + TypeOfChange + '</td><td>Item No.</td><td>' + CHLG."Old Value" + '</td><td>' + CHLG."New Value" + '</td><td>' + username + '</td><td>' + FORMAT(CHLG."Date and Time") + '</td></tr>');
                                                        recCount := recCount + 1;
                                                    END;
                                            ...
                                            ELSE
                                                ...
                                                ELSE
                                                    IF (CHLG."Type of Change" = 1) AND (CHLG."Field No." = 80006) AND (CHLG."Table No." = 37) THEN BEGIN // IF Pending by status change
                                                        ...
                                                        SMTP_MAIL.AppendBody('<tr><td>' + CHLG."Primary Key Field 2 Value" + '</td><td>' + custname + '</td><td>' + CHLG."Primary Key Field 3 Value" + '</td><td>' + ' ' + '</td><td>' + itemNo + '</td><td>' + Desc + '</td>');
                                                        SMTP_MAIL.AppendBody('<td>Pending By Change</td><td>Pending By</td><td>' + OldPendingByVal + '</td><td>' + NewPendingByVal + '</td><td>' + username + '</td><td>' + FORMAT(CHLG."Date and Time") + '</td></tr>');
            ...
            UNTIL CHLG.NEXT = 0;
        //SMTP_MAIL.AppendBody('<td colspan=6 align="center"><b>Total RPOs</b></td><td align="right"><b>'+FORMAT(NoofRPO)+'</b></td></tr>');
        SMTP_MAIL.AppendBody('</table><br>');
        releasedflag := FALSE;
        modifyflag := FALSE;
        custname := '';
                    ...
                    IF releasedflag = TRUE THEN BEGIN
                        ...
                        IF ChangeFlag = TRUE THEN BEGIN
                            ...
                            IF (CHLG."Field No." = 15) AND (CHLG."Table No." = 37) THEN BEGIN
                                IF (CHLG."Type of Change" = 1) AND (CHLG."Old Value" <> '0') THEN BEGIN
                                    IF modifyflag = FALSE THEN BEGIN
                                        SMTP_MAIL.AppendBody('<P>Closed Orders, </P>');
                                        SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:100%; font-size:10pt;"><tr><th >Sale order No.</th><th >Customer</th><th>Line No.</th><th>Schdle Line No.</th><th>Item No.</th>');
                                        SMTP_MAIL.AppendBody('<th>Description</th><th>Type of Change</th><th>Changed Field</th><th>Previous Value</th><th>New Value</th><th>User ID</th><th>Time of Change</th></tr>');
                                        modifyflag := TRUE;
                                    END;
                                    ...
                                    TypeOfChange := 'Quantity Change';
                                    SMTP_MAIL.AppendBody('<tr><td>' + CHLG."Primary Key Field 2 Value" + '</td><td>' + custname + '</td><td>' + CHLG."Primary Key Field 3 Value" + '</td><td>' + ' ' + '</td><td>' + itemNo + '</td><td>' + Desc + '</td><td>' + TypeOfChange + '</td>');
                                    SMTP_MAIL.AppendBody('<td>Quantity</td><td>' + CHLG."Old Value" + '</td><td>' + CHLG."New Value" + '</td><td>' + username + '</td><td>' + FORMAT(CHLG."Date and Time") + '</td></tr>');
                                    recCount := recCount + 1;
                                END
                            END
                            ELSE
                                            ...
                                            IF TypeOfChange <> 'New Item Insertion' THEN BEGIN
                                                IF CHLG."Old Value" <> '' THEN BEGIN
                                                    ...
                                                    IF (COPYSTR(CHLG."Primary Key Field 2 Value", 14, 2) = '/L') OR (COPYSTR(CHLG."Primary Key Field 2 Value", 14, 2) = '/T') THEN
                                                        LMD_Count := 1;
                                                    SMTP_MAIL.AppendBody('<tr><td>' + CHLG."Primary Key Field 2 Value" + '</td><td>' + custname + '</td><td>' + CHLG."Primary Key Field 3 Value" + '</td><td>' + ' ' + '</td><td>' + itemNo + '</td><td>' + Desc + '</td><td>' + TypeOfChange + '</td>');
                                                    SMTP_MAIL.AppendBody('<td>Item No.</td><td>' + CHLG."Old Value" + '</td><td>' + CHLG."New Value" + '</td><td>' + username + '</td><td>' + FORMAT(CHLG."Date and Time") + '</td></tr>');
                                                    recCount := recCount + 1;
                                                END ELSE BEGIN
                                                    IF modifyflag = FALSE THEN BEGIN
                                                        SMTP_MAIL.AppendBody('<P>Closed Orders, </P>');
                            ...
                            IF (CHLG."Field No." = 15) AND (CHLG."Table No." = 37) THEN BEGIN
                                IF (CHLG."Type of Change" = 1) AND (CHLG."Old Value" <> '0') THEN BEGIN
                                    ...
                                    IF newflag = FALSE THEN BEGIN
                                        SMTP_MAIL.AppendBody('<P>Orders Released on ' + FORMAT(headerdate) + ', </P>');
                                        SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:100%; font-size:10pt;"><tr><th >Sale order No.</th><th >Customer</th><th>Line No.</th><th>Item No.</th><th>Description</th><th>Type of Change</th>');
                                        SMTP_MAIL.AppendBody('<th>Changed Field</th><th>Previous Value</th><th>New Value</th><th>User ID</th><th>Time of Change</th></tr>');
                                        newflag := TRUE;
                                    END;
                                    ...
                                    SMTP_MAIL.AppendBody('<tr><td>' + CHLG."Primary Key Field 2 Value" + '</td><td>' + custname + '</td><td>' + CHLG."Primary Key Field 3 Value" + '</td><td>' + itemNo + '</td><td>' + Desc + '</td><td>' + TypeOfChange + '</td>');
                                    SMTP_MAIL.AppendBody('<td>Quantity</td><td>' + CHLG."Old Value" + '</td><td>' + CHLG."New Value" + '</td><td>' + username + '</td><td>' + FORMAT(CHLG."Date and Time") + '</td></tr>');
                                    recCount := recCount + 1;
                                END
                            END
        ...
        SMTP_MAIL.AppendBody('</table><br>');
        SMTP_MAIL.AppendBody('<p align ="left"> Regards,<br>ERP Team </p>');
        SMTP_MAIL.AppendBody('<br><p align = "center">:: Note: Auto Generated mail from ERP</b> :: </b></P></div></body></html>');
        IF LMD_Count > 0 THEN
            SMTP_MAIL.AddRecipients('lmd@efftronics.com');
        IF recCount > 0 THEN
            SMTP_MAIL.Send;
    ...
    begin
        Mail_From := 'erp@efftronics.com';
        Mail_To := 'yesu@efftronics.com,prasanthi@efftronics.com,erp@efftronics.com,susmithal@efftronics.com';
        //Mail_To := 'pranavi@efftronics.com';
        Subject := 'Reg: Billing Deviated AMC Details';
        Body := '';
        SMTP_MAIL.CreateMessage('ERP', Mail_From, Mail_To, Subject, Body, TRUE);
        SMTP_MAIL.AppendBody('<html><head><style> divone{background-color: white; width: 9000px; padding: 20px; border-style:solid ; border-color:#666699;  margin: 20px;} </style></head>');
        SMTP_MAIL.AppendBody('<body><div style="border-color:#666699;  margin: 20px; border-width:15px;   border-style:solid; padding: 20px; width: 800px;"><label><font size="6">Billing Deviated AMC Details</font></label>');
        SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
        SMTP_MAIL.AppendBody('<h>Dear Sir/Madam,</h><br><br>');
        //SMTP_MAIL.AppendBody('<h><b>Responsible Dept: <font color=red>Manufacturing.</b></font></h><br>');
        SMTP_MAIL.AppendBody('<P> The below are details of To Be Shipped AMCs, </P>');
        ...
        IF SL.FINDSET THEN
            REPEAT
                ...
                IF SL."Shipment Date" <> 0D THEN
                    SMTP_MAIL.AppendBody('<td>' + SL.Description + '</td><td align = "center">' + FORMAT(SL."Shipment Date", 0, '<Day,2>-<Month Text,3>-<Year>') + '</td><td align = "right">' + FORMAT(TODAY() - SL."Shipment Date"))
                ELSE
                    SMTP_MAIL.AppendBody('<td>' + SL.Description + '</td><td align = "center"> </td><td align = "right"> ');
                SMTP_MAIL.AppendBody('</td><td align = "right">' + formataddress.ChangeCurrency(ROUND(UnitPrice)) + '</td></tr>');
                recCount := recCount + 1;
                TotalPrice += SL."Unit Price";
            UNTIL SL.NEXT = 0;
    ...
    begin
        ...
        Mail_To := Mail_To + ',pmurali@efftronics.com,bharat@efftronics.com,dineel@efftronics.com,prasanthi@efftronics.com,sales@efftronics.com';
        ...
        IF SIH.FINDSET THEN
            REPEAT
                ...
                SalesPerson := '';
                Division.RESET;
                Division.SETFILTER(Division."Division Code", SIH."Dispatched Location");
        ...
        IF LMD_count > 0 THEN
            Mail_To := Mail_To + ',lmd@efftronics.com';
        SMTP_MAIL.CreateMessage('ERP', Mail_From, Mail_To, Subject, Body, TRUE);
        //MESSAGE(Body);
        IF recCount > 0 THEN
            SMTP_MAIL.Send;
    ...
    begin
        ...
        headerdate := CalMngmt.CalcDateBOC('-1D', TODAY(), 3, 'PROD', '', 3, 'PROD', '', FALSE);
        CSLG.RESET;
    ...
    begin
        ...
        Mail_From := 'erp@efftronics.com';
        Mail_To := 'erp@efftronics.com,store@efftronics.com,controlroom@efftronics.com,temc@efftronics.com,qainward@efftronics.com,dineel@efftronics.com';
        //Mail_To := 'erp@efftronics.com';
            ...
            IF Itm.GET(FORMAT(RecordSet.Fields.Item('No_').Value)) THEN BEGIN
                ...
                BEGIN
                    ...
                    CASE FORMAT(RecordSet.Fields.Item('ShelfLifestatus').Value) OF
                        ...
                        'F_T_Exipired':
                            status_desc := 'Floor Life To Be Expired Items';
                        'S_T_Expired':
                            status_desc := 'Shelf Life To Be Expired Items';
                    END;
                    CASE FORMAT(RecordSet.Fields.Item('ShelfLifestatus').Value) OF
                        'S_Expired':
                            colorcode := 'FF9936';
    ...
    begin
        ...
        Mail_To := '';
        Mail_From := 'erp@efftronics.com';
        Mail_To := 'Padmaja@efftronics.com,controlroom@efftronics.com,anilkumar@efftronics.com,erp@efftronics.com';
        Body := '';
        ...
        */
        ...
        SMTP_MAIL.CreateMessage('ERP', Mail_From, Mail_To, Mail_Subject, Body, TRUE);
        SMTP_MAIL.AppendBody('<html><head><style> divone{background-color: white; width: 1210px; padding: 20px; border-style:solid ; border-color:#666699;  margin: 20px;} </style></head>');
        SMTP_MAIL.AppendBody('<body><div style="border-color:#666699;  margin: 20px; border-width:15px;   border-style:solid; padding: 20px; width: 1110px;"><label><font size="6">To Be Recieved items(Auto Postings)</font></label>');
        SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
        SMTP_MAIL.AppendBody('<h>Dear Sir/Madam,</h><br><br>');
        SMTP_MAIL.AppendBody('<P> Details of Shortage Items, </P>');
        ...
        IF "Material Issues Line".FINDFIRST THEN
            REPEAT
                IF Count <> 0 THEN BEGIN
                    ...
                    IF ("Present BOM" <> "Material Issues Line"."Production BOM No.") AND ("Present Item" <> "Material Issues Line"."Item No.") THEN BEGIN
                        ...
                        IF "Material Issues Header".FINDFIRST THEN BEGIN
                            SMTP_MAIL.AppendBody('<td>' + FORMAT(Req_Qty) + '</td></tr>');
                            Req_Qty := "Material Issues Line"."Qty. to Receive";
                            SMTP_MAIL.AppendBody('<tr><td>' + "Material Issues Header"."Proj Description" + '</td><td>' + "Material Issues Line".Description + '</td>');
                ...
                ELSE BEGIN
                    ...
                    IF "Material Issues Header".FINDFIRST THEN BEGIN
                        Count := 1;
                        SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:100%; font-size:10pt;"><tr><th>PCB Description</th><th> Item Description </th><th>Req_Qty<th></tr>');
                        SMTP_MAIL.AppendBody('<tr><td>' + "Material Issues Header"."Proj Description" + '</td><td>' + "Material Issues Line".Description + '</td>');
                        Req_Qty := "Material Issues Line"."Qty. to Receive";
                    END;
                END;
        ...
        IF Count = 0 THEN
            SMTP_MAIL.AppendBody('No Pendings')
        ELSE BEGIN
            SMTP_MAIL.AppendBody('<td>' + FORMAT(Req_Qty) + '</td></tr>');
    ...
    begin
        ...
        MIH.SETFILTER("Released Date", '>%1 & <%2', 040108D, TODAY);
    ...
    begin
        ...
        SMTP_MAIL.AppendBody('<html><head><style> divone{background-color: white; width:600px; padding: 20px; border-style:solid ; border-color:skyblue;  margin: 10px;} </style></head>');
        SMTP_MAIL.AppendBody('<body><div style="border-color:skyblue;  margin:10px; border-width:10px;   border-style:solid; padding: 20px; width: 600px;"><label><font size="5">Item Stock Details</font></label>');
        SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
        SMTP_MAIL.AppendBody('<h>Dear Sir/Mam ,</h> <br/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Below are the Item details having stock less than the specified threshold value <br/>');
        SMTP_MAIL.AppendBody('<table border="1" style="border-collapse:collapse; width:60%; font-size:10pt;">' +
        ...
        REPEAT
            ...
            IF ileqty < Item.Stock_Threshold_Value THEN BEGIN
                SMTP_MAIL.AppendBody('<tr><td style=width:3%>' + Item."No." + '</td><td style=width:25%;word-wrap:break-word;>' + Item.Description +
        ...
        SMTP_MAIL.AppendBody('</table><br>');
        SMTP_MAIL.AppendBody('<hr style=solid; color= #3333CC>');
        SMTP_MAIL.AppendBody('<b>Regards,<br>ERP</b><br>');
        SMTP_MAIL.AppendBody('<br>********::<i>Note</i>: This is System generated Automail::********</b>');
    ...
    begin
        ...
        SMTP_MAIL.AppendBody('<html><head><style> divone{background-color: white; width: 500px; padding: 20px; border-style:solid ; border-color:#b268ab;  margin: 20px;} </style></head>');
        SMTP_MAIL.AppendBody('<body><div style="border-color:#2980b9;  margin: 20px; border-width:10px;   border-style:solid; padding: 10px; width: 900px;">');
        SMTP_MAIL.AppendBody('<label><font size="6"> Order Verified but Status is in Open Orders List </font></label>');
        SMTP_MAIL.AppendBody('<hr style=solid; color= #c0392b >');
        SMTP_MAIL.AppendBody('<h>Dear Sir/Madam,</h><br>');
        SMTP_MAIL.AppendBody('<P>      These are the List of  Orders that are verified but still in OPEN Status</P>');
        SMTP_MAIL.AppendBody('<table border = "1" style="border-collapse:collapse; width:90%; fond-size:9pt;">');
        SMTP_MAIL.AppendBody('<tr>');
        SMTP_MAIL.AppendBody('<th>Order No </th>');
        SMTP_MAIL.AppendBody('<th>Customer Name</th>');
        SMTP_MAIL.AppendBody('<th>Status</th>');
        SMTP_MAIL.AppendBody('<th>Verified Status</th>');
        SMTP_MAIL.AppendBody('<th>Sale Order Value </th>');
        SMTP_MAIL.AppendBody('</tr>');
        ...
        IF salesheader.FINDSET THEN
            REPEAT
                SMTP_MAIL.AppendBody('<tr>');
                SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(salesheader."No.") + '</td>');
                SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(salesheader."Sell-to Customer Name") + '</td>');
                SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(salesheader.Status) + '</td>');
                SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(salesheader."Order Verified") + '</td>');
                SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(salesheader."Sale Order Total Amount") + '</td>');
                SMTP_MAIL.AppendBody('</tr>');
            UNTIL salesheader.NEXT = 0;
        SMTP_MAIL.AppendBody('</table>');
        SMTP_MAIL.AppendBody('<br>regards,<br>');
        SMTP_MAIL.AppendBody('ERP.<br>');
        ...
        FOR No_of_Managers := 1 TO ARRAYLEN(Managers) DO BEGIN
            ...
            IF ISG.FINDSET THEN BEGIN
                ...
                IF SalesLine.FINDSET THEN BEGIN
                    SMTP_MAIL.CreateMessage('erp', 'erp@efftronics.com', Mail_To, Subject, Body, TRUE);
                    SMTP_MAIL.AppendBody('<html><head><style> divone{background-color: white; width: 2800px; padding: 20px; border-style:solid ; border-color:#b268ab;  margin: 20px;} </style></head>');
                    SMTP_MAIL.AppendBody('<body><div style="border-color:#2980b9;  margin: 15px; border-width:8px;   border-style:solid; padding: 8px; width: 2800px;">');
                    SMTP_MAIL.AppendBody('<label><font size="6">To be Executed Sale Orders List </font></label>');
                    SMTP_MAIL.AppendBody('<hr style=solid; color= #c0392b >');
                    SMTP_MAIL.AppendBody('<h>Dear Sir/Madam,</h><br>');
                    SMTP_MAIL.AppendBody('<P>        These are the List of  Orders that are in Pending from R&D Side</P>');
                    SMTP_MAIL.AppendBody('<table border = "1" style="border-collapse:collapse; width:100%; fond-size:5pt;">');
                    SMTP_MAIL.AppendBody('<tr>');
                    SMTP_MAIL.AppendBody('<th> Order Number </th>');
                    SMTP_MAIL.AppendBody('<th> Item </th>');
                    SMTP_MAIL.AppendBody('<th> Description </th>');
                    SMTP_MAIL.AppendBody('<th> Product </th>');
                    SMTP_MAIL.AppendBody('<th> Qty to Ship </th>');
                    SMTP_MAIL.AppendBody('<th> Vertical </th>');
                    SMTP_MAIL.AppendBody('<th> Main Category </th>');
                    SMTP_MAIL.AppendBody('<th> Sub Category </th>');
                    SMTP_MAIL.AppendBody('<th> Reason </th>');
                    SMTP_MAIL.AppendBody('<th> Remarks </th>');
                    //SMTP_MAIL.AppendBody('<th> Incharge </th>');
                    SMTP_MAIL.AppendBody('</th>');
                    IF ISG1.FINDSET THEN
                        REPEAT
                            SalesLine1.RESET;
                            ...
                            IF SalesLine1.FINDSET THEN
                                ...
                                BEGIN
                                    ...
                                    SMTP_MAIL.AppendBody('<tr>');
                                    SMTP_MAIL.AppendBody('<td  >' + ' ' + FORMAT(SalesLine1."Document No.") + '</td>');
                                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesLine1."No.") + '</td>');
                                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesLine1.Description) + '</td>');
                                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesLine1.ProductGroup) + '</td>');
                                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesLine1."Outstanding Quantity") + '</td>');
                                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesLine1.Vertical) + '</td>');
                                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesLine1.MainCategory) + '</td>');
                                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesLine1.SubCategory) + '</td>');
                                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesLine1.Reason) + '</td>');
                                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesLine1.Remarks) + '</td>');
                                    //SMTP_MAIL.AppendBody('<td >'+' '+FORMAT(US."Full Name")+'</td>');
                                    SMTP_MAIL.AppendBody('</tr>');
                                END
                                UNTIL SalesLine1.NEXT = 0;
                        UNTIL ISG1.NEXT = 0;
                    SMTP_MAIL.AppendBody('</table><br>');
                    SMTP_MAIL.AppendBody('For report: <a href =' + '"https://app.powerbi.com/view?r=eyJrIjoiNTc0ZmQzNTYtNmJhMi00NWMwLThmMzQtYThmNjRhM2VhZWI0IiwidCI6IjZhZDY1ZDZkLWZkODctNDQ5OC04ZTkyLTUzNGM3YTA3ZjlmOCIsImMiOjEwfQ%3D%3D"' + '> click here </a><br>');
                    SMTP_MAIL.AppendBody('<br>Regards,<br>');
    ...
    begin
        ...
        IF Sh.FINDSET THEN BEGIN
            ...
            IF SL1.FINDSET THEN BEGIN
                ...
                SMTP_MAIL.CreateMessage('erp', 'erp@efftronics.com', Mail_To, Subject, Body, TRUE);
                SMTP_MAIL.AppendBody('<html><head><style> divone{background-color: white; width: 1800px; padding: 8px; border-style:solid ; border-color:#E36015;  margin: 10px;} </style></head>');
                SMTP_MAIL.AppendBody('<body><div style="border-color:#18BEAF;  margin: 8px; border-width:10px;   border-style:solid; padding: 8px; width: 1800px;">');
                ...
                SMTP_MAIL.AppendBody('<P>        Below are the List of Pre Expected Orders of IREPS Customer</P>');
                SMTP_MAIL.AppendBody('<table border = "1" style="border-collapse:collapse; width:100%; fond-size:5pt;">');
                SMTP_MAIL.AppendBody('<tr>');
                SMTP_MAIL.AppendBody('<th> Order No </th>');
                SMTP_MAIL.AppendBody('<th> Sales Person </th>');
                SMTP_MAIL.AppendBody('<th> Tender No. </th>');
                SMTP_MAIL.AppendBody('<th> Tender Floated by </th>');
                SMTP_MAIL.AppendBody('<th>  Amount </th>');
                SMTP_MAIL.AppendBody('<th> Item </th>');
                SMTP_MAIL.AppendBody('<th> Quantity </th>');
                SMTP_MAIL.AppendBody('<th> Tender Due Dt </th>');
                SMTP_MAIL.AppendBody('<th> Deviated Days </th>');
                SMTP_MAIL.AppendBody('<th>  Zone </th>');
                SMTP_MAIL.AppendBody('<th>  Division </th>');
                SMTP_MAIL.AppendBody('<th> Remarks </th>');
                SMTP_MAIL.AppendBody('</th>');
                REPEAT
                BEGIN
                    IF Sh."Tender Due Date" < TODAY() THEN BEGIN
                        ...
                        IF SalesPerson.FINDFIRST THEN
                            //SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' '+ FORMAT(SalesPerson.Name)+'</td>');
                            //SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' '+FORMAT(Sh."Customer OrderNo.")+'</td>');
                            // SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' '+FORMAT(Sh."Bill-to Name")+'</td>');  // added for Tender Floated by Field
                            SalesLine.RESET;
                        ...
                        IF SalesLine.FINDSET THEN BEGIN
                            REPEAT
                                ...
                                SMTP_MAIL.AppendBody('<tr>');
                                SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(COPYSTR(Sh."No.", 9, 10)) + '</td>');
                                SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(SalesPerson.Name) + '</td>');
                                SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(Sh."Customer OrderNo.") + '</td>');
                                SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(Sh."Bill-to Name") + '</td>');


                                SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(NeedtoInvAmt) + '</td>');
                                //SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' '+FORMAT(Sh."Tender Published Date")+'</td>');
                                SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(SalesLine.Description) + '</td>');
                                SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(SalesLine.Quantity) + '</td>');
                                SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(Sh."Tender Due Date") + '</td>');
                                SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(TODAY() - Sh."Tender Due Date") + '</td>');
                                Division.RESET;
                                ...
                                IF Division.FINDFIRST THEN BEGIN
                                    ...
                                    IF Zone.FINDFIRST THEN BEGIN
                                        SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(Zone."Zone Name") + '</td>');
                                        SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(Division."Division Name") + '</td>');
                                    END;
                                END;

                                //SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' '+FORMAT('Please convert it to Order')+'</td>');  // dynamic remarks removed and sale sheader remarks updated as per the Request of Sales
                                SMTP_MAIL.AppendBody('<td Style = "color:red">' + ' ' + FORMAT(Sh.Remarks) + '</td>');
                                SMTP_MAIL.AppendBody('</tr>');
                    ...
                    ELSE BEGIN
                        ...
                        IF SalesLine.FINDSET THEN BEGIN
                            REPEAT
                                ...
                                NeedtoInvAmt := ROUND(NeedtoInvAmt + (SalesLine."Unitcost(LOA)" * SalesLine."Outstanding Quantity"), 2);
                                SMTP_MAIL.AppendBody('<tr>');
                                SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(COPYSTR(Sh."No.", 9, 10)) + '</td>');
                                SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesPerson.Name) + '</td>');
                                SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(Sh."Customer OrderNo.") + '</td>');
                                SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(Sh."Bill-to Name") + '</td>'); // added for Tender Floated by Field

                                SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(NeedtoInvAmt) + '</td>');
                                SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesLine.Description) + '</td>');
                                SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(SalesLine.Quantity) + '</td>');
                                SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(Sh."Tender Due Date") + '</td>');
                                SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT('') + '</td>');
                                Division.RESET;
                                Division.SETFILTER("Division Code", Sh."Railway Division");
                                IF Division.FINDFIRST THEN BEGIN
                                    Zone.RESET;
                                    Zone.SETFILTER("Zone Code", Division."Zone code");
                                    IF Zone.FINDFIRST THEN BEGIN
                                        SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(Zone."Zone Name") + '</td>');
                                        SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(Division."Division Name") + '</td>');
                                    END;
                                END;
                                //SMTP_MAIL.AppendBody('<td>' + ' '+FORMAT('Make followups')+'</td>'); // dynamic remarks removed and sale sheader remarks updated as per the Request of Sales
                                SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(Sh.Remarks) + '</td>');
                                SMTP_MAIL.AppendBody('</tr>');
                            UNTIL SalesLine.NEXT = 0;
                        END;
                    END;
                ...
                UNTIL Sh.NEXT = 0;
                SMTP_MAIL.AppendBody('</table><br>');
                SMTP_MAIL.AppendBody('<br/> <i><span Style = "color:red" >Red Color Orders are Tender Due Date need to Change Items</Span></i>');
                SMTP_MAIL.AppendBody('For report: <a href =' + '"https://app.powerbi.com/view?r=eyJrIjoiNDk0ZjdjMjMtYzg5Ny00MGI5LWIzMmQtZTc1NzJiYzkxMWMzIiwidCI6IjZhZDY1ZDZkLWZkODctNDQ5OC04ZTkyLTUzNGM3YTA3ZjlmOCIsImMiOjEwfQ%3D%3D"' + '> click here </a><br>');
                SMTP_MAIL.AppendBody('<br>Regards,<br>');
                SMTP_MAIL.AppendBody('ERP.');
                SMTP_MAIL.AppendBody('<br>');
                SMTP_MAIL.AppendBody('<div style="Background-color:#F35317; color:#F0F1F5; "><p align= "center"> *****AUTO GENERATED MAIL FROM ERP**** </div><br/></div>');
                SMTP_MAIL.Send;
            END;
        END;
    ...
    begin
        ...
        FOR No_of_Managers := 1 TO Day - 1 DO BEGIN
            ...
            SMTP_MAIL.AppendBody('<html><head><style> divone{background-color: white; width: 1200px; padding: 8px; border-style:solid ; border-color:#E36015;  margin: 8px;} </style></head>');
            SMTP_MAIL.AppendBody('<body><div style="border-color:#18BEAF;  margin: 8px; border-width:10px;   border-style:solid; padding: 8px; width: 1100px;">');
            SMTP_MAIL.AppendBody('<label><font size="6">Calibration</font></label>');
            SMTP_MAIL.AppendBody('<hr style=solid; color= #2C2EA8 >');
            SMTP_MAIL.AppendBody('<h>Hi Mr./Ms. ' + FORMAT(OwnersArr[No_of_Managers]) + ' ,</h><br>');
            SMTP_MAIL.AppendBody('<P>        These are the List of  Equipments that are going to be Calibrated</P>');
            SMTP_MAIL.AppendBody('<table border = "1" style="border-collapse:collapse; width:100%; fond-size:5pt;">');
            SMTP_MAIL.AppendBody('<tr>');
            SMTP_MAIL.AppendBody('<th> Equipment Number(ERP) </th>');
            SMTP_MAIL.AppendBody('<th> Description </th>');
            SMTP_MAIL.AppendBody('<th> Model </th>');
            SMTP_MAIL.AppendBody('<th> Serial Number </th>');
            SMTP_MAIL.AppendBody('<th> Last Calibr Dt</th>');
            SMTP_MAIL.AppendBody('<th> Next Calibr Dt</th>');
            SMTP_MAIL.AppendBody('<th> Delay in Calibr(D)</th>');
            //SMTP_MAIL.AppendBody('<th> Owner Name </th>');
            //SMTP_MAIL.AppendBody('<th> Life Time of Equipment</th>');
            //SMTP_MAIL.AppendBody('<th> Last Transaction Number</th>');
            //SMTP_MAIL.AppendBody('<th> Current User </th>');
            SMTP_MAIL.AppendBody('</tr>');
            ...
            IF CalibrationEntr.FINDSET THEN
                ...
                BEGIN
                    SMTP_MAIL.AppendBody('<tr>');
                    //SMTP_MAIL.AppendBody('<td >'+' '+FORMAT(CalibrationEntr."Item No")+'</td>');
                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(CalibrationEntr."Equipment No") + '</td>');
                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(CalibrationEntr.Description) + '</td>');
                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(CalibrationEntr."Model No.") + '</td>');
                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(CalibrationEntr."Eqpt. Serial No.") + '</td>');
                    SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(CalibrationEntr."Last Calibration Date") + '</td>');
                    IF CalibrationEntr."Next Calibration Due On" < TODAY() THEN BEGIN
                        SMTP_MAIL.AppendBody('<td Style = "color:red" >' + ' ' + FORMAT(CalibrationEntr."Next Calibration Due On") + '</td>');
                        SMTP_MAIL.AppendBody('<td Style = "color:red" >' + ' ' + FORMAT(TODAY() - CalibrationEntr."Next Calibration Due On") + '</td>');
                    END
                    ELSE BEGIN
                        SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(CalibrationEntr."Next Calibration Due On") + '</td>');
                        //SMTP_MAIL.AppendBody('<td >'+' '+FORMAT(TODAY()-CalibrationEntr."Next Calibration Due On")+'</td>');
                        SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(0) + '</td>');
                    ...
                    END;
                    //SMTP_MAIL.AppendBody('<td >'+' '+FORMAT(CalibrationEntr."Owner of the Equpmnt_empid")+'</td>');
                    //SMTP_MAIL.AppendBody('<td >'+' '+FORMAT(CalibrationEntr."Life time in Yrs")+'</td>');
                    ...
                    BEGIN
                      //SMTP_MAIL.AppendBody('<td >'+' '+FORMAT(ile."Document No.")+'</td>');
                      PMIH1.RESET;
                      PMIH1.SETCURRENTKEY("No.");
                      PMIH1.SETFILTER("No.",ILE."Document No.");
                      IF PMIH1.FINDFIRST THEN
                        BEGIN
                          SMTP_MAIL.AppendBody('<td >'+' '+FORMAT(COPYSTR(PMIH1."User ID",12))+'</td>');
                        END;
                      END;
                      */
                    SMTP_MAIL.AppendBody('</tr>');
            ...
            SMTP_MAIL.AppendBody('<br/> <i><span Style = "color:red" >Red Color Indicates Deviated Items</Span></i><br>');

            SMTP_MAIL.AppendBody('<br/> <span  >For any Queries reply to  </Span>' + '<a href="calibration@efftronics.com"<b>calibration@efftronics.com</b></a></br>');
    ...
    begin
        ...
        WHILE NOT RecordSet1.EOF DO BEGIN
            ...
            IF IRHeader.FINDSET THEN BEGIN
                ...
                SMTP_MAIL.AppendBody('<label><font size="6">QC Flag</font></label>');
                ...
                SMTP_MAIL.AppendBody('<th> Item Number </th>');
                SMTP_MAIL.AppendBody('<th> Description </th>');
                SMTP_MAIL.AppendBody('<th> Posting Date </th>');
                SMTP_MAIL.AppendBody('<th> Posting Date to Today(D) </th>');
                SMTP_MAIL.AppendBody('<th> Concerned Dept</th>');
                SMTP_MAIL.AppendBody('<th> Action To be Taken</th>');
                SMTP_MAIL.AppendBody('<th> Item Grp </th>');
                SMTP_MAIL.AppendBody('</tr>');
                REPEAT
                BEGIN
                    SMTP_MAIL.AppendBody('<tr style = "border: 2px solid black">');
                    SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(IRHeader."Item No.") + '</td>');
                    SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(IRHeader."Item Description") + '</td>');
                    SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(IRHeader."Posted Date") + '</td>');
                    SMTP_MAIL.AppendBody('<td align="right">' + ' ' + FORMAT(TODAY() - IRHeader."Posted Date") + '</td>');
                    SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(IRHeader."Concerned Dept") + '</td>');
                    SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(IRHeader."Action To Be Taken") + '</td>');
                    item1.RESET;
                    item1.SETFILTER("No.", IRHeader."Item No.");
                    IF item1.FINDFIRST THEN
                        SMTP_MAIL.AppendBody('<td >' + ' ' + FORMAT(item1."Product Group Code") + '</td>');
                END;
                UNTIL IRHeader.NEXT = 0;
                SMTP_MAIL.AppendBody('</table><br>');
                //SMTP_MAIL.AppendBody('<br/> <i><span Style = "color:red" >Red Color Indicates Deviated Items</Span></i><br>');
                SMTP_MAIL.AppendBody('<br/> <span  >For any Queries reply to  </Span>' + '<a href="calibration@efftronics.com"<b>calibration@efftronics.com</b></a></br>');
                SMTP_MAIL.AppendBody('For report: <a href ="https://app.powerbi.com/view?r=eyJrIjoiOGQ0Mzk2MDEtMTRkNi00ZjA5LTkxMmUtZGZiZjYxYjQ2MmQyIiwidCI6IjZhZDY1ZDZkLWZkODctNDQ5OC04ZTkyLTUzNGM3YTA3ZjlmOCIsImMiOjEwfQ%3D%3D"> click here </a><br>');
                SMTP_MAIL.AppendBody('<br>Regards,<br>');
                SMTP_MAIL.AppendBody('ERP.');
                SMTP_MAIL.AppendBody('<br>');
    ...
    begin
        ...
        IF bg.FINDSET THEN BEGIN
            ...
            IF (Mail_From <> '') AND (Mail_To <> '') THEN BEGIN
                ...
                SMTP_MAIL.AppendBody('<table border = "1" style="border-collapse:collapse; width:100%; fond-size:5pt;border: 3px solid black">');
                SMTP_MAIL.AppendBody('<tr style ="border: 2px solid black" bgcolor="#7B92BC">');
                SMTP_MAIL.AppendBody('<th> BG Number </th>');
                SMTP_MAIL.AppendBody('<th> Customer Name </th>');
                SMTP_MAIL.AppendBody('<th> Customer Ord.No </th>');
                SMTP_MAIL.AppendBody('<th> BG Value </th>');
                SMTP_MAIL.AppendBody('<th> Expire Date </th>');
                SMTP_MAIL.AppendBody('<th> Extension Date </th>');
                SMTP_MAIL.AppendBody('<th> Department </th>');
                SMTP_MAIL.AppendBody('<th> Order No </th>');
                SMTP_MAIL.AppendBody('</tr>');
                //SMTP_MAIL.AddAttachment(Attachment,'');
                REPEAT
                    ...
                    IF STRLEN(bg."BG No.") >= 15 THEN BEGIN
                        SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(bg."BG No.") + '</td>');
                        IF CUSTOMER.GET(bg."Issued to/Received from") THEN
                            SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(CUSTOMER.Name) + '</td>');
                        SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(bg."Customer Order No.") + '</td>');
                        SMTP_MAIL.AppendBody('<td align="right">' + ' ' + FORMAT(bg."BG Value") + '</td>');
                        SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(bg."Expiry Date") + '</td>');
                        SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(bg."Claim Date") + '</td>');
                        SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(COPYSTR(bg."Doc No.", 5, 3)) + '</td>');
                        SMTP_MAIL.AppendBody('<td>' + ' ' + FORMAT(bg."Doc No.") + '</td>');
                    END;
                UNTIL bg.NEXT = 0;
                SMTP_MAIL.AppendBody('</table><br>');
                SMTP_MAIL.AppendBody('<br>Regards,<br>');
