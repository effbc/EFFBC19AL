tableextension 70126 ProductionBOMHeaderExt extends "Production BOM Header"
{
    fields
    {
        ...
        modify(Status)
        {
            ...
            begin
                ...
                if (Rec."Creation Date" <= 20200909D) and (xRec.Status = 1) and not (UserId in ['EFFTRONICS\VANIDEVI', 'EFFTRONICS\VISHNUPRIYA']) then
    ...
    BEGIN
        ItemLoc.Get(ItemNoParam);
        // Added by Mnraju
        if (ItemLoc."Product Group Code" = 'FPRODUCT') or (ItemLoc."Product Group Code" = 'CPCB') then begin
        ...
        if ProdBomLineLoc.Find('-') then begin
            ...
            if ProdBOMHeader.FindFirst then begin
                if not Item.Get(No) then begin
                    ...
                    if (Item."Product Group Code" = 'FPRODUCT') or (Item."Product Group Code" = 'CPCB') then begin
            ...
            repeat
                ...
                if Item.Get(ProdBomLineLoc."No.") then begin
                    if not (Item."Product Group Code" in ['FPRODUCT', 'CPCB']) then begin
    ...
    PROCEDURE ValidateStatus(Authorised_ID_LVar: Code[50]; BOM: Code[20]; StatusLVAr: 'New,Certified,"Under Development",Closed');
    VAR
