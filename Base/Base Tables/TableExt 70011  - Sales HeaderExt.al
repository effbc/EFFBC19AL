    ...
    BEGIN
        ...
        IF FORMAT(SalesHeader."Document Type") IN ['Order', 'Amc'] THEN BEGIN
            ...
            IF Updated = TRUE THEN BEGIN
                ...
                ROMail.AppendBody('<label><font size="6"> Order Information</font></label>');
                ROMail.AppendBody('<hr style=solid; color= #3333CC>');
                ROMail.AppendBody('<h>Dear Sir/Madam,</h><br><br>');
                ROMail.AppendBody('<table border="1" style="border-collapse:collapse; width:100%; font-size:10pt;">');
                ROMail.AppendBody('<tr><td><b>Order Type</b></td><td>' + FORMAT(SalesHeader."Document Type") + '</td></tr>');
                ROMail.AppendBody('<tr><td><b>Order No  </b></td><td>' + SalesHeader."No." + '</td></tr>');
                ROMail.AppendBody('<tr><td><b>Customer No.  </b></td><td>' + SalesHeader."Sell-to Customer No." + '</td></tr>');
                ROMail.AppendBody('<tr><td><b>Customer Name  </b></td><td>' + SalesHeader."Sell-to Customer Name" + '</td></tr>');
                ROMail.AppendBody('<tr><td><b>Customer Posting Group </b></td><td>' + SalesHeader."Customer Posting Group" + '</td></tr>');
                ROMail.AppendBody('<tr><td><b>Remarks   </b></td><td>' + SalesHeader.Remarks + '</td></tr>');
                UserGRec.RESET;
                UserGRec.SETRANGE(UserGRec.EmployeeID, "Salesperson Code");//B2B
                IF UserGRec.FINDFIRST THEN
                    ROMail.AppendBody('<tr><td><b>Sales Executive  </b></td><td>' + FORMAT(UserGRec."Full Name") + '</td></tr>');
                ROMail.AppendBody('</Table>');
                ROMail.AppendBody('<BR><p align ="left"> <b>Note::</b> Order Status is updated to Y in CashFLow.Please check any other to be closed invoice entries still exist if so please close</p>');
