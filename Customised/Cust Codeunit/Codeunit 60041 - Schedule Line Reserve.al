codeunit 60041 "Schedule Line Reserve"
{
        ...
        IF HasError THEN
          ...
          THEN BEGIN
            IF (NewScheduleComp."Line No." <> OldScheduleComp."Line No.") THEN BEGIN
              ReservMgt.SetDelChallanLine(OldScheduleComp);
              ReservMgt.DeleteReservEntries(TRUE,0);
              ReservMgt.SetDelChallanLine(NewScheduleComp);
            END ELSE BEGIN
              ReservMgt.SetDelChallanLine(NewScheduleComp);
              ReservMgt.DeleteReservEntries(TRUE,0);
            END;
    ...
    begin
        WITH ScheduleComp DO BEGIN
          ...
          ReservMgt.SetDelChallanLine(ScheduleComp);
    ...
    begin
        ...
        WITH NewScheduleComp DO BEGIN
          IF "Line No." = OldScheduleComp."Line No." THEN
            IF "Quantity(Base)" = OldScheduleComp."Quantity(Base)" THEN
              EXIT;
          IF "Line No." = 0 THEN
            IF NOT ScheduleComp.GET("No.","Line No.") THEN
              EXIT;

          ReservMgt.SetDelChallanLine(NewScheduleComp);
          IF "Qty. per Unit of Measure" <> OldScheduleComp."Qty. per Unit of Measure" THEN
            ReservMgt.ModifyUnitOfMeasure;

          IF "Outstanding Qty.(Base)" * OldScheduleComp."Outstanding Qty.(Base)" < 0 THEN
