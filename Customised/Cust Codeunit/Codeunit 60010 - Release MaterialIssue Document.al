codeunit 60010 "Release MaterialIssue Document"
{
    ...
    begin
        ...
        BEGIN
          ...
          BEGIN
            IF user.Dept <> 'STR' THEN
              IF user.levels= FALSE THEN
              BEGIN
                IF NOT(("Transfer-from Code"='CAL') AND (USERID='EFFTRONICS\VIJAYALAKSHMIB')) THEN
                  ERROR('YOU HAVE NO PERMISSION TO AUTHORIZE');
              END;
          END;
        END;
        ...
        IF NOT ( (Status = Status::Open) OR (Status = Status::"Sent for Authorization") ) THEN
        ...
        VALIDATE("Released DateTime",CREATEDATETIME("Released Date",Release_time));
        IF "Authorized Date"=0D THEN //UPGREV2.0
          "Authorized Date":=WORKDATE;//UPGREV2.0
        MODIFY;
        //UPGREV2.0>>
        /*
        ...
        */
        ...
        j:=0;
        //Mail_Body:=
        MaterialIssueLine.SETRANGE(MaterialIssueLine."Document No.","No.");
        ...
        OR (MaterialIssueLine."Item No."='METOLGN00034') OR (MaterialIssueLine."Item No."='METOLGN00086')) THEN
        BEGIN
        ...
        IF ("Transfer-to Code"='STR')OR ("Transfer-to Code"='DAMAGE') THEN
        IF j=1 THEN
        BEGIN
    ...
    begin
        IF USERID IN ['EFFTRONICS\ANILKUMAR','EFFTRONICS\MARY','EFFTRONICS\ANANDA','EFFTRONICS\RATNA','EFFTRONICS\DMADHAVI','EFFTRONICS\PADMASRI',
                      'EFFTRONICS\RAMADEVI','EFFTRONICS\VSNGEETHA','EFFTRONICS\NAGALAKSHMI','EFFTRONICS\SITARAJYAM','EFFTRONICS\SUJANI','EFFTRONICS\BHAVANIP',
                      'EFFTRONICS\GRAVI','EFFTRONICS\VISHNUPRIYA','EFFTRONICS\PKOTESWARARAO','EFFTRONICS\DURGAMALLESWARI','EFFTRONICS\TLAKSHMI','EFFTRONICS\TULASI','EFFTRONICS\RENUKACH','EFFTRONICS\RRAHUL','EFFTRONICS\B2BOTS']  THEN
        BEGIN
    ...
    begin
        ...
        IF "Material Issues Header"."Transfer-to Code"='CST' THEN
        BEGIN
        /* TESTFIELD("Service Order No.");
          TESTFIELD("Service Item");   */
        END;
    ...
    begin
        IF MIH.Status<>MIH.Status::Released THEN
        BEGIN
          IF USERID IN ['EFFTRONICS\ANILKUMAR','EFFTRONICS\ANANDA','EFFTRONICS\MARY','EFFTRONICS\RATNA','EFFTRONICS\PADMASRI','EFFTRONICS\SUJANI',
                        'EFFTRONICS\PADMA','EFFTRONICS\SIRISHA','EFFTRONICS\VIJAYALAKSHMIB','EFFTRONICS\RENUKAH']  THEN
          BEGIN
            ...
            WHILE NOT RecordSet.EOF DO
              BEGIN
                ...
                IF (MIH."No."=FORMAT(RecordSet.Fields.Item('requestid').Value))  THEN
                BEGIN
                  IF MIH."No."<>'' THEN
                  BEGIN
