codeunit 33000253 "Inspection Jnl.-Post Line"
{
        ...
        BEGIN
          IF Item.GET("Item No.") THEN BEGIN
            IF ItemTrackingCode.GET(Item."Item Tracking Code") THEN BEGIN
              ...
              END ELSE IF (ItemTrackingCode."Lot Specific Tracking") OR (ItemTrackingCode."Lot Purchase Inbound Tracking") OR
    ...
    begin
        ...
        ELSE
          ...
          REPEAT
          IF (PIL."Receipt No." <> text) AND (PIL."Receipt No." <> '') THEN
          BEGIN
            c:=PIL.COUNT;
            text:=PIL."Receipt No.";
          ...
          IF (InspectRcpt."Source Type" = "Source Type" :: "In Bound") AND (NOT InspectRcpt."Quality Before Receipt") AND
            (InspectRcpt."Qty. To Vendor (Rework)" <> 0) THEN BEGIN
             IF InspectRcpt."Rework Level" = 0 THEN BEGIN
               ...
               ItemJnlLine."Quality Ledger Entry No." := "Item Ledger Entry No.";
               //B2B QC Check Start
               ItemJnlLine."QC Check" := TRUE;
               //B2B QC Check End
               ItemJnlLine.INSERT;
    ...
    begin
        ...
        IF DeliveryChalan3.GET(InspectRcpt."Rework Reference Document No.") THEN
         IF QualityItemLedgEntry.GET(DeliveryChalan3."In bound Item Ledger Entry No.") THEN
           BEGIN
            IF RemainQty <= QualityItemLedgEntry."Remaining Quantity" THEN
              QtyToApply := RemainQty
            ELSE
          ...
          IF DeliveryChalan.GET("Rework Reference Document No.") THEN BEGIN
            IF QualityItemLedgerEntry.GET(DeliveryChalan."Item Ledger Entry No.") THEN BEGIN
               IF Vend.GET("Vendor No.") THEN
                 QualityItemLedgerEntry."Location Code" :="Location Code";
    ...
    begin
        ...
        IF PRL.FINDSET THEN
          REPEAT
          BEGIN
            IF PRL."Quantity (Base)" = (PRL."Quantity Accepted" + PRL."Quantity Rejected") THEN
              BEGIN
                  PRL."QC Passed":=TRUE;
                  PRL.MODIFY;
              END;
