codeunit 60011 "MaterialIssueOrde-Post Receipt"
{
    ...
    begin
        ...
        */
        IF NOT(USERID IN ['EFFTRONICS\NAGANAVEEN','EFFTRONICS\NVVRAO','EFFTRONICS\SUDHAKARREDDY','EFFTRONICS\RABBANI','EFFTRONICS\MADHAVIP','EFFTRONICS\HARIK','EFFTRONICS\BHARGAV',
                      'EFFTRONICS\TIRUPATAIAH','EFFTRONICS\VARAPRASAD','EFFTRONICS\PRAVEENA','EFFTRONICS\TNMRAO','EFFTRONICS\BHASKAR','EFFTRONICS\SRIKANTH','EFFTRONICS\SURESHPNV','EFFTRONICS\PRADEEPS']) THEN
        BEGIN
          IF (("Transfer-from Code"='OLD STOCK') AND ("Transfer-to Code"='SITE')) OR (("Transfer-from Code"='SITE') AND ("Transfer-to Code"='OLD STOCK')) THEN
            ERROR('U Dont have permissions');
        END;
        RequistionNo := "No.";
        IF NOT (("Transfer-to Code" = 'SITE') AND ("Transfer-from Code" <> 'CS')) THEN
        BEGIN
        /*
        ...
        */
        ...
        IF NOT(USERID IN ['EFFTRONICS\RAMADEVI','EFFTRONICS\BHAVANIP','EFFTRONICS\SRIVALLI','EFFTRONICS\NAGALAKSHMI','EFFTRONICS\VISHNUPRIYA']) THEN
          BEGIN
            IF (("Transfer-from Code"='SITE') AND ("Transfer-to Code"='CS')) OR (("Transfer-from Code" = 'CS') AND ("Transfer-to Code"='SITE')) THEN
        ...
        IF BulkPosting = FALSE THEN
        BEGIN
            IF NOT CONFIRM(Text009,FALSE,"No.") THEN
              EXIT;
        END;
        ...
        WITH MaterialIssueHeader DO BEGIN
          ...
          IF (FORMAT("BOM Type") IN['Mechanical','Wiring','Testing','Packing']) THEN
          BEGIN
            MaterialIssueLine.RESET;
            MaterialIssueLine.SETRANGE("Document No.","No.");
            MaterialIssueLine.SETFILTER(Quantity,'<>0');
            ...
            REPEAT
              ...
              REPEAT
                ...
                "Mech&Wiring_Items".SETFILTER("Mech&Wiring_Items"."BOM Type",FORMAT("BOM Type"));
                IF NOT "Mech&Wiring_Items".FINDFIRST THEN
                BEGIN
                  ...
                  "Mech&Wiring_Items"."BOM Type":=FORMAT("BOM Type");
                  "Mech&Wiring_Items"."Request No.":="No.";
          ...
          */
          ...
          PostedMaterialIssueHeader."Auto Post" := "Auto Post";
          PostedMaterialIssueHeader."Issued DateTime":=CURRENTDATETIME;//B2B1.0
          PostedMaterialIssueHeader."Transfer-from Code" := "Transfer-from Code";
          PostedMaterialIssueHeader."Transfer-from Name" := "Transfer-from Name";
          PostedMaterialIssueHeader."Transfer-from Name 2" := "Transfer-from Name 2";
          PostedMaterialIssueHeader."Transfer-from Address" := "Transfer-from Address";
          PostedMaterialIssueHeader."Transfer-from Address 2" := "Transfer-from Address 2";
          PostedMaterialIssueHeader."Transfer-from Post Code" := "Transfer-from Post Code";
          PostedMaterialIssueHeader."Transfer-from City" := "Transfer-from City";
          PostedMaterialIssueHeader."Transfer-from County" := "Transfer-from County";
          PostedMaterialIssueHeader."Transfer-from Country Code" := "Transfer-from Country Code";
          PostedMaterialIssueHeader."Transfer-from Contact" := "Transfer-from Contact";
          PostedMaterialIssueHeader."Transfer-to Code" := "Transfer-to Code";
          PostedMaterialIssueHeader."Transfer-to Name" := "Transfer-to Name";
          PostedMaterialIssueHeader."Transfer-to Name 2" := "Transfer-to Name 2";
          PostedMaterialIssueHeader."Transfer-to Address" := "Transfer-to Address";
          PostedMaterialIssueHeader."Transfer-to Address 2" := "Transfer-to Address 2";
          PostedMaterialIssueHeader."Transfer-to Post Code" := "Transfer-to Post Code";
          PostedMaterialIssueHeader."Transfer-to City" := "Transfer-to City";
          PostedMaterialIssueHeader."Transfer-to County" := "Transfer-to County";
          PostedMaterialIssueHeader."Transfer-to Country Code" := "Transfer-to Country Code";
          PostedMaterialIssueHeader."Transfer-to Contact" := "Transfer-to Contact";
          PostedMaterialIssueHeader."Posting Date" := "Posting Date";
          PostedMaterialIssueHeader."Receipt Date" := "Receipt Date";
          PostedMaterialIssueHeader."Posted By" := USERID;
          PostedMaterialIssueHeader."Shortcut Dimension 1 Code" := "Shortcut Dimension 1 Code";
          PostedMaterialIssueHeader."Shortcut Dimension 2 Code" := "Shortcut Dimension 2 Code";
          //DIM1.0
          PostedMaterialIssueHeader."Dimension Set ID" := "Dimension Set ID";
          //DIM1.0
          PostedMaterialIssueHeader."Material Issue No." := "No.";
          PostedMaterialIssueHeader."External Document No." := "External Document No.";
          PostedMaterialIssueHeader."Prod. Order No." := "Prod. Order No.";
          PostedMaterialIssueHeader."Prod. Order Line No.":= "Prod. Order Line No.";
          PostedMaterialIssueHeader."Prod. BOM No." := "Prod. BOM No.";
          PostedMaterialIssueHeader."Sales Order No." := "Sales Order No.";
          PostedMaterialIssueHeader."Service Order No." := "Service Order No.";
          PostedMaterialIssueHeader."Resource Name" := "Resource Name";
          PostedMaterialIssueHeader."User ID" := "User ID";
          PostedMaterialIssueHeader."Required Date" := "Required Date";
          PostedMaterialIssueHeader."Released Date" := "Released Date";
          PostedMaterialIssueHeader."Released Time":="Released Time";
          PostedMaterialIssueHeader."Released By"  := "Released By";
          PostedMaterialIssueHeader."Reason Code" := "Reason Code";
          PostedMaterialIssueHeader.Remarks:=Remarks;
          PostedMaterialIssueHeader."Service Order No.":="Service Order No.";
          PostedMaterialIssueHeader."Service Item":="Service Item";
          PostedMaterialIssueHeader."Service Item Serial No.":= "Service Item Serial No.";
          PostedMaterialIssueHeader."Service Item Description":= "Service Item Description";
          PostedMaterialIssueHeader."No. Series" := InvtSetup."Posted Transfer Rcpt. Nos.";
          PostedMaterialIssueHeader."No." :=
              NoSeriesMgt.GetNextNo(
                InvtSetup."Posted Material Issue Nos.","Posting Date",TRUE);
          // pranavi
          ...
          //pranavi
          PostedMaterialIssueHeader."Transaction ID":="Transaction ID";
          PostedMaterialIssueHeader."Customer No":="Customer No"; // Added by Pranavi on 27-Jan-2016 for LED Cards process
          ...
          */
          //DIM1.0 End
          CopyCommentLines(4,5,"No.",PostedMaterialIssueHeader."No.");
            ...
            REPEAT
              ...
              BEGIN
                 ...
                 // PostedMaterialIssueLine."Unit Cost":=MaterialIssueLine."Unit Cost";
                  ...
                  PostedMaterialIssueLine."Prod. Order No." := "Prod. Order No.";
                  PostedMaterialIssueLine."Prod. Order Line No." := "Prod. Order Line No.";
                  PostedMaterialIssueLine.Saleorderno:="Sales Order No.";
                 ...
                 /* IF (MaterialIssueHeader."Transfer-to Code" = 'STR') AND
                  ...
                  IF MaterialIssueLine."Qty. to Receive" > 0 THEN
                  BEGIN
        ...
        //      ItemJnlPostLine.RunWithCheck(TempItemJnlLine,TempJnlLineDim2);
          ...
          HeaderDeleted := DeleteOrder(MaterialIssueHeader,MaterialIssueLine);
        END;
        ...
        IF (PMI_No <> '') AND (ToCode = 'SITE') AND (FromCode <> 'CS')  THEN
        BEGIN
          COMMIT;
          PMIGRec.RESET;
    ...
    begin
        ...
        ItemJnlLine."Product Group Code" := MaterialIssueLine."Product Group Code";
    ...
    begin
        ...
        WITH MaterialIssueHeader DO BEGIN
          ...
          */
          ...
          PostedMaterialIssueHeader."Shortcut Dimension 1 Code" := "Shortcut Dimension 1 Code";
          PostedMaterialIssueHeader."Shortcut Dimension 2 Code" := "Shortcut Dimension 2 Code";
          PostedMaterialIssueHeader."Dimension Set ID" :=   "Dimension Set ID"; //DIM1.0
          PostedMaterialIssueHeader."Material Issue No." := "No.";
          PostedMaterialIssueHeader."External Document No." := "External Document No.";
          PostedMaterialIssueHeader."Prod. Order No." := "Prod. Order No.";
          PostedMaterialIssueHeader."Prod. Order Line No.":= "Prod. Order Line No.";
    ...
    begin
          IF Returned THEN
          BEGIN
            IF Itm.GET(MILine."Item No.") AND (Itm.MSL <> 0) AND NOT (Itm."Floor Life at 25 C 40% RH" IN['','INFINITE']) THEN
            BEGIN
              MSL_ILE.RESET;
              ...
              BEGIN
                ...
                BEGIN
                  ...
                  BEGIN
                    ...
                    BEGIN
                      MSL_ILE."Floor Life"+=DateAndTime.DateDiff('H',PMI."Issued DateTime",CURRENTDATETIME,DayofWeekInput,WeekofYearInput);
                      MSL_ILE.MODIFY;
                    END;
                  END;
                END;
          ...
          BEGIN
            ...
            BEGIN
              ...
              BEGIN
                ...
                IF (DateAndTime.DateDiff('N',MILine."MBB Packet Open DateTime",MILine."MBB Packet Close DateTime",DayofWeekInput,WeekofYearInput)) > 15 THEN
                BEGIN
                  MSL_ILE."Floor Life"+=ROUND((DateAndTime.DateDiff('N',MILine."MBB Packet Open DateTime",MILine."MBB Packet Close DateTime",DayofWeekInput,WeekofYearInput))/60,0.01);
                  MSL_ILE.MODIFY;
                END;
              END;
            END;
    ...
    begin
        ...
        descbuff :='';
        FOR i:=1 TO STRLEN(IssuesLine.Description) DO
         BEGIN
