...
{
    // version ESPL 1.0,Rev01,Eff01,Eff02
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './Material Availability.rdlc';
    ...
    dataset
    {
        dataitem("Prod. Order Component";"Prod. Order Component")
        {
            ...
            begin
                ...
                Shortgvalue := 0;
                Alternate_Buffer := 0;
                IF Previous_Plan_Date>0D THEN
                  BEGIN
                     IF Previous_Plan_Date <> "Prod. Order Component"."Production Plan Date" THEN
                      BEGIN
                ...
                IF Dum.GET("Prod. Order Component"."Item No.") THEN
                  BEGIN // item item present in dum begin
                   ...
                   //Added by Pranavi on 26-Dec-2015 for not considering expected orders items if lead time <=15 days
                    ...
                    PO1.SETRANGE(PO1."No.","Prod. Order Component"."Prod. Order No.");
                      IF PO1.FIND('-') THEN
                        BEGIN // PO1 BEGIN
                          ...
                          IF Itm.FINDFIRST THEN
                             BEGIN // Itm Begin
                               IF USERID <> 'EFFTRONICS\PRANAVI' THEN
                                 IF (COPYSTR(PO1."Sales Order No.",5,3) = 'EXP') AND (FORMAT(Itm."Safety Lead Time") IN ['2D','4D','7D','15D']) THEN
                                    BEGIN
                      ...
                      IF ExpFlag = FALSE THEN
                        BEGIN // exp flag false begin
                          IF Item_Req.GET("Prod. Order Component"."Item No.") THEN
                            BEGIN
                          ...
                          ELSE
                            BEGIN
                              ...
                              Item_Req.INSERT;
                            END;
                          IF Dum."Maximum Inventory">=("Prod. Order Component"."Expected Quantity"- "Qty. In Issues & Req")  THEN
                              BEGIN
                                Dum."Maximum Inventory":=Dum."Maximum Inventory"-("Prod. Order Component"."Expected Quantity"-"Qty. In Issues & Req");
                                Dum.MODIFY;
                              END
                          ELSE
                           BEGIN // max invent < expected qty begin
                             //  IF "Prod. Order Component"."Item No." ='ECDIOPN00120' THEN
                             ...
                             // IF ALTERNATE ITEM STOCK IS ALSO NOT THERE, THEN SHORTAGE DETAILS WILL BE UPDATED
                               IF NOT AVB THEN
                                  BEGIN // not avb begin
                                    Dum.RESET;
                                    IF Dum.GET("Prod. Order Component"."Item No.") THEN
                                      BEGIN // if item in dum begin
                                          IF FORMAT(Dum."Safety Lead Time") <>'' THEN
                                            BEGIN // safety lead time not null begin
                                              ...
                                              IF "Planned Purchase Date"<WORKDATE THEN
                                                BEGIN // planned purchase date < work date begin
                                              ...
                                              IF Item_Lot.FIND('-') THEN
                                                  BEGIN
                                                     ...
                                                     // Item_Lot.Shortage+=("Prod. Order Component"."Expected Quantity"-("Qty. In Issues & Req"+Alternate_Buffer));
                                                      IF Alternate_Buffer > 0 THEN
                                                          BEGIN
                                                              IF Alternate_Buffer >= Dum."Maximum Inventory" THEN
                                                                 BEGIN
                                                  ...
                                                  END
                                              ELSE
                                                BEGIN // if not find in ILE begin
                                                  Item_Lot.INIT;
            ...
            begin
                ...
                */
                  ...
                  REPEAT
                    ...
                    IF "Indent line".FIND('-') THEN
                      BEGIN
                        ...
                        IF "Indent line"."Indent Status"="Indent line"."Indent Status"::Indent THEN
                          BEGIN
                ...
                // Added by Pranavi 23-Feb-2016 for checking mail send to vendor for pos
                  Auto:=FALSE;
                  IF EVALUATE(PODateVar,'01-01-2016') = TRUE THEN
                    BEGIN
                          Mail_count := 0;
                          PurchHeadr.RESET;
                          ...
                          REPEAT
                            ...
                            BEGIN
                              ...
                              BEGIN
                                Mail_count+=1;
                                IF Polist = '' THEN   Polist := PurchHeadr."No."
                                ELSE  Polist+=', '+PurchHeadr."No.";
                              END;
                            END;
                        ...
                        END
                  ELSE ERROR('Date Format Mis match! Please contach ERP Team!');
                END;
                // End by Pranavi
                ...
                // IF ANY OF THE ABOVE INFORMATION MISSED , ERROR WILL RETURNED
                  ...
                  REPEAT
                    ...
                    BEGIN
                      ...
                      CurrReport.BREAK;
                    END;
                    IF PO."Prod Start date">PO."Planned Dispatch Date" THEN
                    BEGIN
        ...
        dataitem("Sales Line";"Sales Line")
        {
            ...
            begin
                ...
                BEGIN
                  ...
                  IF TempBoutStockTable.FINDFIRST THEN
                  BEGIN
                    ...
                    ELSE RemPMIQty := 0;
                    IF RemPMIQty >= "Sales Line"."Outstanding Quantity" THEN
                    BEGIN
                      NewOutStndingQty := 0;
                  ...
                  BEGIN
                    ...
                    REPEAT
                      ...
                      IF PostedMatIssLinGRec.FINDFIRST THEN
                      BEGIN
                  ...
                  BEGIN
                    IF Dum."Maximum Inventory">=(NewOutStndingQty)  THEN
                    BEGIN
                         ...
                         // "Planned Purchase Date" := WORKDATE +1; // BY SUJANI ON 28-AUG 1:17 PM
                          ...
                          BEGIN
                            ...
                            BEGIN
                              ...
                              IF Dum_Purch_Line."Qty. to Receive">NewOutStndingQty THEN
                              BEGIN
                ...
                BEGIN
                  ...
                  BEGIN
                    ...
                    BEGIN
                      ...
                      ELSE RemPMIQty := 0;
                      IF RemPMIQty >= Schedule2."Outstanding Qty." THEN
                      BEGIN
                        NewOutStndingQty := 0;
                   ...
                   /* IF Schedule."No."='ECBOUBT00004' THEN
                    BEGIN
                      ...
                      MESSAGE(Dum_Po);
                    END;  */
                    IF Item_Req.GET(Schedule2."No.") THEN
                    BEGIN
                      Item_Req."Required Quantity"+=NewOutStndingQty;
                      Item_Req."Req Qty"+=NewOutStndingQty;
                      Item_Req.MODIFY;
        ...
        {
            DataItemTableView = WHERE(Safety Stock Quantity=FILTER(>0),Product Group Code=FILTER(<>FPRODUCT&<>CPCB));
            ...
            begin
                ...
                */
                ...
                IF NOT( Dum.GET("Saftey Items"."No.") ) THEN
                BEGIN
                ...
                IF Dum.GET("Saftey Items"."No.") THEN
                BEGIN
                IF "Saftey Items"."Safety Stock Quantity">Dum."Maximum Inventory" THEN
                BEGIN
          ...
          BEGIN
            ...
            BEGIN
              ...
              BEGIN
                ...
                IF Item_Req.GET("Material Issues Line"."Item No.") THEN
                BEGIN
    ...
    begin
        ...
        IF Alter_PO.FIND('-') THEN
        BEGIN
          // FINDING THE ITEMSUBGROUP CODE
          IF Item.GET(Alter_PO."Source No.") THEN
          BEGIN
            ...
            REPEAT
              ...
              IF  Dum.GET("Alternative Items"."Alternative Item No.") THEN
              BEGIN
                IF Dum."Maximum Inventory">0 THEN
                BEGIN
                  IF Dum."Maximum Inventory">=Req_Qty THEN
                  BEGIN
    ...
    begin
        //IF item1 = 'ECDIOPN00120' THEN
          ...
          IF Item.FIND('-') THEN
          BEGIN
            IF (Item."Product Group Code"<>'CPCB') AND (Item."Product Group Code"<>'FPRODUCT') THEN
            BEGIN
                  ...
                  //  "Planned Purchase Date" := WORKDATE + 1;
                    ...
                    BEGIN
                      ...
                      BEGIN
                        ...
                        BEGIN
                        REPEAT
                          IF (Dum_Purch_Line."Qty. to Receive">NewOutStndingQty) AND (NOT AVB)  THEN
                          BEGIN
                            Dum_Purch_Line."Qty. to Receive"-=NewOutStndingQty;
                            Dum_Purch_Line.MODIFY;
                            Possible_Procured_Date:=Dum_Purch_Line."Deviated Receipt Date"+2;
    ...
    begin
                  ...
                  BEGIN
                    ...
                    IF Item_Lot.Shortage >0 THEN
                      Item_Lot.INSERT;
                    IF Item_Req.GET(Shortage_Item) THEN
                    BEGIN
    ...
    begin
        ...
        BEGIN
          ...
          BEGIN
            ...
            BEGIN
              IF (MSL_ILE."MFD Date" <> 0D) AND (Itm."Component Shelf Life(Years)">0) THEN
              BEGIN
              ...
              BEGIN
                ...
                BEGIN
                  IF NOT (Itm."Floor Life at 25 C 40% RH" IN['',' ','INFINITE']) THEN
                  BEGIN
