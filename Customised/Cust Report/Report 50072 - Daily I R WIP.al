report 50072 "Daily I R WIP"
{
    // version B2B 1.0

    DefaultLayout = RDLC;
    RDLCLayout = './Daily I R WIP.rdlc';

    dataset
    {
        dataitem("Inspection Receipt Header"; "Inspection Receipt Header")
        {
            DataItemTableView = SORTING(Document Date, Source Type, Vendor No., Prod. Order No.) WHERE(Source Type=CONST(WIP),Status=CONST(No));
