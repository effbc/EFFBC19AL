report 80016 "Item Wise LAR3"
{
    // version NAVW17.00,Rev01,Eff02

    DefaultLayout = RDLC;
    RDLCLayout = './Item Wise LAR3.rdlc';
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING(Field2,Field3,Field25,Field24,Field16) ORDER(Ascending) WHERE(Field4=FILTER(>0), Field3=CONST(STR));
            ...
            begin
                IF PrevItem1 <> Table50005."Item No." THEN BEGIN
                  PrevItem1 := Table50005."Item No." ;
                  // Copy code from // Mat.Issue Track. Specification, GroupHea - OnPostSection() >>
                  ITEM_TB.GET(Table50005."Item No.");
                  // Copy code from // Mat.Issue Track. Specification, GroupHea - OnPostSection() <<
                END;
            end;
            ...
            begin
                ...
                IF Table50005.GETFILTER(Table50005."Item No.")<>'' THEN
                Choice:=Choice::Item;


                IF Reserve_Item<>'' THEN
                Table50005.SETRANGE(Table50005."Item No.",Reserve_Item);
                PrevItem1 := '';
            end;
        }
        dataitem("Production Order Shortage Item";"Production Order Shortage Item")
        {
            DataItemTableView = SORTING(Field2,Field8) ORDER(Ascending);
            column(TODAY;TODAY)
            {
            }
            ...
            begin
                ...
                PO1.RESET;
                PO1.SETFILTER(PO1."No.",Table33000901."Production Order");
                ...
                IF ((InsertPrevShortageAt <>'') AND (InsertPrevShortageAt <> Table33000901."Shortage At" )) OR
                                     ((InsertPrevItem <> '') AND (InsertPrevItem <> Table33000901.Item )) THEN BEGIN
                    /*
                    ...
                    UNTIL QILE.NEXT=0;
                    */
                    IF (Create_MCH_REQ) AND (InsertPrevShortageAt='MCH') THEN
                    BEGIN
                    ...
                    IF (Create_PRD_REQ) AND (Table33000901."Shortage At"='PRODSTR') THEN       //SUNDAR
                    BEGIN
                      ITEM_TB.GET(InsertPrevItem);
                      MATERIAL_ISSUES_LINE.INIT;
                      MATERIAL_ISSUES_LINE."Document No." :=MATERIAL_ISSUES_HEADER."No." ;
                    ...
                    AUTH_SHORT_ITEMS.Shortage:=Table33000901.Shortage;
                    AUTH_SHORT_ITEMS.INSERT;
                END;
                //Insert Hack
                // copy code from // Production Order Shortage Item, GroupFoo - OnPreSection() <<
                
                IF PrevShortageAt <> Table33000901."Shortage At" THEN BEGIN
                   PrevShortageAt := Table33000901."Shortage At";
                   ShortageGroupTotal := 0;
                END;
                
                ShortageGroupTotal += Table33000901.Shortage;
                
                InsertPrevShortageAt := '';
                InsertPrevItem := '';
                InsertPrevItemDesc := '';
                
                InsertPrevShortageAt := Table33000901."Shortage At";
                InsertPrevItem := Table33000901.Item;
                InsertPrevItemDesc := Table33000901.Description;
                ...
                QILE.SETCURRENTKEY(QILE."Posting Date",QILE."Item No.");
                QILE.SETRANGE(QILE."Item No.",InsertPrevItem);
                QILE.SETRANGE(QILE."Inspection Status",QILE."Inspection Status"::"0");
                QILE.SETRANGE(QILE."Sent for Rework",FALSE);
                QILE.SETFILTER(QILE."Remaining Quantity",'>%1',0);
                QILE.SETRANGE(QILE."Location Code",'STR');
                QILE.SETRANGE(QILE.Accept,TRUE);
                ...
                UNTIL QILE.NEXT=0;
                ItemGrec.RESET;
                IF ItemGrec.GET(InsertPrevItem) THEN
                BEGIN
                 // Leadtime:=0;
        ...
        {
            DataItemTableView = SORTING(Field60001,Source No.,Field90081) ORDER(Ascending) WHERE(Status=CONST(Released), Field90081=FILTER(<>''), Field90087=CONST(Yes));
    ...
    var
        ...
        AUTH_SHORT_ITEMS : Record "Authorized Shortage Items";
