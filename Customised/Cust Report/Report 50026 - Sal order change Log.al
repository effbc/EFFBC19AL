...
{
    DefaultLayout = RDLC;
    RDLCLayout = './Sal order change Log.rdlc';

    dataset
    {
        dataitem("<Change Log Entry>";"Change Log Entry")
        {
            DataItemTableView = SORTING(Table No.,Primary Key Field 1 Value) ORDER(Ascending) WHERE(Table No.=FILTER(37|60025),Type of Change=FILTER(<>Insertion),Primary Key Field 1 Value=FILTER(1|4),Field No.=FILTER(5|6|7|11|15|18|153));
            ...
            dataitem("<Change Log Entry2>";"Change Log Entry")
            {
                ...
                begin
                      ...
                      BEGIN
                        ...
                        IF CLE.FIND('-') THEN
                        REPEAT
                          IF CLE."Field No."=5 THEN
                          BEGIN
                            Item_old:=CLE."Old Value";
                            Item_new:=CLE."New Value";
                          END
                          ELSE IF CLE."Field No."=6 THEN
                          BEGIN
                            Desc_old:=CLE."Old Value";
                            Desc_new:=CLE."New Value";
                          END
                          ELSE IF CLE."Field No."=7 THEN
                          BEGIN
                            Qty_old:=CLE."Old Value";
                            Qty_new:=CLE."New Value";
                          END;
