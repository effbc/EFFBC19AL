report 50036 "Item Replacement In BOM"
{
    ...
    dataset
    {
        dataitem("Integer";"Integer")
        {
            ...
            begin
                IF USERID IN ['EFFTRONICS\JHANSI','EFFTRONICS\VANIDEVI','EFFTRONICS\VISHNUPRIYA','EFFTRONICS\SUJANI','EFFTRONICS\ANILKUMAR'] THEN
                BEGIN
                  ...
                  BEGIN
                    ...
                    IF CONFIRM('Item Present IN '+FORMAT(PBL.COUNT)+' BOMS AND.\Item is Present in '+FORMAT(POL.COUNT)+' RPOs.\Do You want to Replace?') THEN
                    BEGIN
                      IF PBL.FINDSET THEN
                      REPEAT
                        ...
                        IF BOMType <> '' THEN
                        BEGIN
                          IF Item.GET(PBL."Production BOM No.") THEN
                          BEGIN
                              IF Item."Item Sub Group Code" <> BOMType THEN
                              BEGIN
                        ...
                        BEGIN
                          ...
                          IF (PBH.FINDFIRST) AND ( BOMTypeChecking = TRUE) THEN
                          BEGIN
                            //added by sujani to give bom replacements based on bom group (master bom or production bom or pcb)
                              testvar:=COPYSTR(PBH."No.",1,8)<>'ECMPBPCB';
                               IF ((("Standard BOM"=TRUE) AND (PBH."Stranded BOM"=TRUE)) OR (("Master BOM"=TRUE) AND (COPYSTR(PBH."No.",1,8)='ECMPBPCB')) OR
                                (("Production BOM"=TRUE )AND  (PBH."Stranded BOM"=FALSE)) AND (testvar))  THEN
                              BEGIN  /// start of the Begin
                                  ...
                                  BEGIN
                                    ...
                                    PBLTemp.MODIFY;
                                  END;
                                  IF (Recertify=TRUE) AND (testcert=TRUE) THEN
                                  BEGIN
                        ...
                        ELSE BEGIN    // Added by Pranavi on 25-Dec-2015
                          ...
                          IF (PBVersion.FINDFIRST) AND (BOMTypeChecking = TRUE) THEN
                          BEGIN
                            IF PBVersion.Status=PBVersion.Status::Certified THEN
                            BEGIN
                              testcert:=TRUE;
                    ...
                    END
                  ELSE IF Operation = Operation::Delete THEN
                  BEGIN
                    ...
                    POL.SETFILTER(POL.Status,'%1',POL.Status::Released);
                    POL.SETFILTER(POL."Item No.",No[1]);
                    IF CONFIRM('Item Present IN '+FORMAT(PBL.COUNT)+' BOMS AND.\Item is Present in '+FORMAT(POL.COUNT)+' RPOs.\Do You want to Delete?') THEN
                    BEGIN
                      IF PBL.FINDSET THEN
                      REPEAT
                        BOMTypeChecking := TRUE;
                    ...
                    group("BOM Details")
                    {
                        ...
                        field(BOMNO;BOMNO)
                        {
                            ...
                            begin
                                ...
                                IF ( NOT (ProdBOMHeader.FINDSET)) THEN
                                BEGIN
