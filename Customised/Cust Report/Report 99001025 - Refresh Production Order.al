...
{
    // version NAVW19.00.00.45778,Rev01,Eff02

    CaptionML = ENU='Refresh Production Order',
                ENN='Refresh Production Order';
    ProcessingOnly = true;
        ...
        dataitem("Production Order";"Production Order")
        {
            ...
            begin
                ...
                IF CalcLines THEN BEGIN
                  IF NOT CreateProdOrderLines.Copy("Production Order",Direction,'',FALSE,AlternatePCBReplace) THEN
                    ErrorOccured := TRUE;
                END ELSE BEGIN
                ...
                END;
                  ...
                  REPEAT
                    ...
                    BEGIN
                      IF Item."Invert Solder type" =TRUE THEN
                      BEGIN
                        IF Item."Type of Solder"=Item."Type of Solder"::DIP  THEN
                          ProdOrderComp."Type of Solder2":='SMD'
                        ELSE IF  (Item."Type of Solder"=Item."Type of Solder"::SMD) THEN
                    ...
                    IF ProdOrderComp."BOM Type"=ProdOrderComp."BOM Type"::Mechanical THEN
                    BEGIN
                      ProdOrderComp.Dept:='SHF';
                      ProdOrderComp.MODIFY;
                    END
                    ELSE IF ProdOrderComp."BOM Type"=ProdOrderComp."BOM Type"::Wiring THEN
                    BEGIN
                    ...
                    BEGIN
                      ...
                      BEGIN
                        ...
                        ELSE IF ProdOrderComp."Material Required Day" = 1 THEN
                    ...
                    group(Warehouse)
                    {
                        ...
                        field(CreateInbRqst;CreateInbRqst)
                        {
                            CaptionML = ENU='Create Inbound Request',
                                        ENN='Create Inbound Request';
                        }
                    }
                    group("Alternate PCB")
                    {
                        CaptionML = ENU='Alternate PCB',
                                    ENN='Warehouse';
                        field(AlternatePCBReplace;AlternatePCBReplace)
                        {
                            CaptionML = ENU='AlternatePCBReplace',
                                        ENN='Create Inbound Request';
                        }
                    }
                }
    ...
    var
        Text000 : TextConst ENU='Refreshing Production Orders...\\',ENN='Refreshing Production Orders...\\';
        Text001 : TextConst ENU='Status         #1##########\',ENN='Status         #1##########\';
        Text002 : TextConst ENU='No.            #2##########',ENN='No.            #2##########';
        Text003 : TextConst ENU='Routings must be calculated, when lines are calculated.',ENN='Routings must be calculated, when lines are calculated.';
        Text004 : TextConst ENU='Component Need must be calculated, when lines are calculated.',ENN='Component Need must be calculated, when lines are calculated.';
        ...
        Text005 : TextConst ENU='One or more of the lines on this %1 require special warehouse handling. The %2 for these lines has been set to blank.',ENN='One or more of the lines on this %1 require special warehouse handling. The %2 for these lines has been set to blank.';
        DeletePickedLinesQst : TextConst Comment='Production order no.: Components for production order 101001 have already been picked. Do you want to continue?',ENU='Components for production order %1 have already been picked. Do you want to continue?',ENN='Components for production order %1 have already been picked. Do you want to continue?';
