report 50092 Cancellation_report_new
{
    ...
    dataset
    {
        ...
        dataitem("Prod. Order Component";"Prod. Order Component")
        {
            ...
            {
                DataItemLink = Source ID=FIELD(Prod. Order No.),Source Prod. Order Line=FIELD(Prod. Order Line No.),Item No.=FIELD(Item No.);
        ...
        dataitem("<Prod. Order Line1>";"Prod. Order Line")
        {
            ...
            {
                DataItemLink = Order No.=FIELD(Prod. Order No.),Item No.=FIELD(Item No.);
                DataItemTableView = WHERE(Entry Type=FILTER(Output),Location Code=FILTER(PROD),Remaining Quantity=FILTER(>0),Open=CONST(Yes),Positive=CONST(Yes));
                dataitem("<Reservation Entry2>";"Reservation Entry")
                {
                    DataItemLink = Item No.=FIELD(Item No.),Serial No.=FIELD(Serial No.);
                    DataItemTableView = SORTING(Source ID,Source Ref. No.,Source Type,Source Subtype,Source Batch Name,Source Prod. Order Line,Reservation Status,Shipment Date,Expected Receipt Date) WHERE(Reservation Status=FILTER(Reservation),Source Type=FILTER(32));
    ...
    trigger OnPreReport();
    begin
        IF NOT(UPPERCASE(USERID) IN ['EFFTRONICS\GRAVI','EFFTRONICS\SUJANI','EFFTRONICS\ANUSHAB','EFFTRONICS\VIJAYA','EFFTRONICS\GURULAKSHMI']) THEN
