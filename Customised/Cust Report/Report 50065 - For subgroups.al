report 50065 "For subgroups"
{
    ...
    dataset
    {
        dataitem("Item Ledger Entry";"Item Ledger Entry")
        {
            ...
            begin
                ...
                BEGIN
                  ...
                  BEGIN
                    IF (PMIHNEW."Transfer-from Code" IN['CS STR','STR']) AND (PMIHNEW."Shortcut Dimension 2 Code"="Item Ledger Entry"."Global Dimension 2 Code") THEN
                    BEGIN
                ...
                PMIHNEW.SETFILTER(PMIHNEW."No.","Item Ledger Entry"."Document No.");
                IF PMIHNEW.FINDFIRST THEN
                  IF (((PMIHNEW."Transfer-from Code" IN['CS STR','STR']) AND (conformation = TRUE)) OR (NOT (PMIHNEW."Transfer-from Code" IN['CS STR','STR']))) THEN
                  BEGIN
                    ...
                    IF PrevItemNo <> "Item Ledger Entry"."Item No." THEN BEGIN
                      ...
                      BEGIN
                        ...
                        IF PostedMaterialIssuesLine.FINDFIRST THEN
                        BEGIN
            ...
            begin
                ...
                BEGIN
                  ...
                  IF  PMIHNEW.FINDFIRST THEN
                  BEGIN
                    IF (PMIHNEW."Transfer-from Code" IN ['CS STR','STR']) AND (PMIHNEW."Shortcut Dimension 2 Code" = prevGDC) THEN
                    BEGIN
                      conformation:=CONFIRM('There is also a material posted from CS STR! \ Would you like to Post That Also or Not! ',FALSE);
                      flag:=TRUE;
                    END;
                ...
                IF PMIHNEW.FINDFIRST THEN
                  ...
                  BEGIN
                    ...
                    BEGIN
                      ...
                      END;
                      Item_Data1:=DELSTR(Item_Data1,STRLEN(Item_Data1),1);
                      IF ItemNewRec."Item Tracking Code"<>'LOT' THEN
            ...
            begin
                ...
                //MNRAJU FOR TIN NO. CHANGING
                   ...
                   // IF ("Authorised By"=3) OR  ("Authorised By">7) THEN
                    ...
                    BEGIN
                      ...
                      ELSE Text_Mng:='Project Manager';
                      AuthName := User."Full Name";
                ...
                BEGIN
                  p2:=TRUE;
                 // MESSAGE('sTART');
                  IF NOT (DC_HEADER.GET("Outward No.")) THEN
                  BEGIN
                    DC_HEADER.INIT;
                    ...
                    User.RESET;
                    User.SETRANGE("User Name","Send To");
                    IF User.FINDFIRST THEN;
                    DC_HEADER."Reciver Name":=User."Full Name";
                   ...
                   // DC_HEADER.Indented:=FORMAT("Authorised By");
                    ...
                    IF User.FINDFIRST THEN;
                    DC_HEADER."Indented Name":=User."Full Name";
                    DC_HEADER.Indented:=FORMAT(User."User Name");
                  ...
                  BEGIN
                    ...
                    BEGIN
                      ...
                      BEGIN
                      DC_HEADER.Indented:=FORMAT(User."User Name");
                      DC_HEADER."StoreIncharge Name":=FORMAT(User."User Name");
                      END;
                      DC_HEADER.Remarks:=FORMAT(Purpose);
                      DC_HEADER."Mode Of Transport":=FORMAT("Mode Of Transport");
                      DC_HEADER."Location Code":="Item Ledger Entry".GETFILTER("Item Ledger Entry"."Global Dimension 2 Code");
        ...
        dataitem("Integer";"Integer")
        {
            ...
            begin
                ...
                IF (NOT p2) AND (Choice=Choice::PostPrint) THEN
                BEGIN
                Start_Date:=DMY2DATE(1,DATE2DMY(TODAY,2),DATE2DMY(TODAY,3));
                "Final Date":=DMY2DATE(Days_Of_Month,DATE2DMY(TODAY,2),DATE2DMY(TODAY,3));
                YearValue := COPYSTR(FORMAT(DATE2DMY(WORKDATE,3)),3,2);
                ...
                // "Outward No.":='CUS/DR/'+Month+'/'+"Outward No.";
                  ...
                  BEGIN
                    ...
                    IF User.FINDFIRST THEN
                      DC_HEADER."StoreIncharge Name":=User."Full Name";

                    //DC_HEADER.Indented:="Authorised By";
                    User.RESET;
                    User.SETRANGE(EmployeeID,incharge_id_no);
                    IF User.FINDFIRST THEN
                       DC_HEADER.Indented:=User."User Name";
                      DC_HEADER."Indented Name":= User."Full Name";
                    DC_HEADER.Remarks:=FORMAT(Purpose);
                    DC_HEADER."Mode Of Transport":=FORMAT("Mode Of Transport");
                    DC_HEADER."Location Code":="Item Ledger Entry".GETFILTER("Item Ledger Entry"."Global Dimension 2 Code");
                ...
                group(Control1102152001)
                {
                    ...
                    field(AMC_Orders;AMC_Orders)
                    {
                        ...
                        begin
                            ...
                            IF PAGE.RUNMODAL(45,AMCGRec) = ACTION :: LookupOK THEN
                            BEGIN
    ...
    begin
        ...
        IF (FORMAT("Item Ledger Entry".GETFILTER("Item Ledger Entry"."Global Dimension 2 Code")) = 'LED-GEN') AND (SendToLoc = SendToLoc::Site) THEN
        BEGIN
          IF Site = '' THEN
        ...
        IF User.FIND('-') THEN
        BEGIN
          Handovered_Person:=User."Full Name";
        ...
        //Begin
        IF NOT (FORMAT("Item Ledger Entry".GETFILTER("Item Ledger Entry"."Global Dimension 2 Code")) IN['','LED-GEN']) THEN
        BEGIN
          IF DivGRec.GET(FORMAT("Item Ledger Entry".GETFILTER("Item Ledger Entry"."Global Dimension 2 Code"))) THEN
            IF ZoneGRec.GET(DivGRec."Zone code") THEN
              AMC_Loc := ZoneGRec."Zone Name"+'('+DivGRec."Division Name"+')'
            ELSE
        ...
        BEGIN
          IF  ("Dimension Value".Code = 'LED-GEN') AND (SendToLoc = SendToLoc::Customer) THEN
          BEGIN
            IF CustGRec.GET(CustrNo) THEN
            BEGIN
              IF FORMAT("Mode Of Transport")<>'By Hand' THEN
              BEGIN
    ...
    var
        ...
        VijayawadaCaption_Control1102154038Lbl : Label 'Vijayawada';
