report 33000071 "Payroll Statutory Post - PF"
{
    ...
    begin
        IF PFPaid = 0 THEN ERROR(Text009);
        ...
        IF PFPaid <> 0 THEN
          BEGIN
            ...
            IF PayElements.GET('PF') THEN PayElements.TESTFIELD("GL Code");
            PayPayrollFunctions.GenLdgEntry(PayElements."GL Code", PostingDate,
                                            DocumentNo,GLDesc, -(PFPaid), '',GLEntryType::Debit,PostingDate);
          END;

        IF PFAdminInsPaid <> 0 THEN
          BEGIN
            GLDesc := 'Salary Entry ,' + FORMAT(MonthNo) + FORMAT(Year);
            PayCompanyPolicy.TESTFIELD("Admin_Ins Charges Payable Acc");
            PayPayrollFunctions.GenLdgEntry(PayCompanyPolicy."Admin_Ins Charges Payable Acc",PostingDate,
                                            DocumentNo,GLDesc, PFAdminInsPaid, '',GLEntryType::Debit,PostingDate);
          END;
        ...
        PayPayrollFunctions.GenLdgEntry(BankGlAccount, PostingDate, DocumentNo,
                                        GLDesc, -(-(PFPaid) + PFAdminInsPaid) ,GlobalBatch, GLEntryType, PostingDate);
        ...
        PayPayrollFunctions.InsertintoBankledgerEntry(-(-(PFPaid) + PFAdminInsPaid),PostingDate,DocumentNo,BatchName,
    ...
    begin
        ...
        IF PayElements.FIND('-') THEN
          BEGIN
            ...
            PayElements.CALCFIELDS(PayElements.TotalEmployerCont2);
            PFPaid := PayElements.SumOfPayableAmount;
            IF PFPaid <> 0 THEN
              PFPaid := PayElements.SumOfPayableAmount - PayElements.TotalEmployerCont - PayElements.TotalEmployerCont2
          END;

        IF PFPaid <> 0 THEN BEGIN
          ...
          PayEmployeePayDetails.SETFILTER("Pay Element Code",'PF|VPF|VPF-C');
          PayEmployeePayDetails.CALCSUMS("Employer Contribition");
          PayEmployeePayDetails.CALCSUMS("Employer Contribition2");
          //PFPaid := ABS(PayEmployeePayDetails."Employer Contribition") + ABS(PayEmployeePayDetails."Employer Contribition2");

          PayEmployeePayDetails.CALCSUMS(Salary);

          PayCompanyPolicy2.RESET;
          PayCompanyPolicy2.SETRANGE(Type,PayCompanyPolicy2.Type::PF);
          PayCompanyPolicy2.SETFILTER("Effective Date",'<=%1', CALCDATE('1M-1D',DMY2DATE(1,MonthNo,Year)));
          IF PayCompanyPolicy2.FIND('-') THEN
            BEGIN
              PFAdminInsPaid := ABS(PayEmployeePayDetails.Salary) * (PayCompanyPolicy2."PF Admin Charges"/100);

              IF PayCompanyPolicy2.EDLI > 0  THEN
                PFAdminInsPaid := PFAdminInsPaid + ((PayEmployeePayDetails."EPS Salary") * (PayCompanyPolicy2.EDLI/100))
              ELSE
                PFAdminInsPaid := PFAdminInsPaid + 0;

              IF PayCompanyPolicy2."Inspection Charges (IDLI)" > 0  THEN
                PFAdminInsPaid := PFAdminInsPaid + ((PayEmployeePayDetails."EPS Salary") *
                                  (PayCompanyPolicy2."Inspection Charges (IDLI)"/100))
              ELSE
                PFAdminInsPaid := PFAdminInsPaid + 0;
            END
          ELSE
    ...
    begin
        ...
        PayEmployeePayDetails.SETRANGE(Month,MonthNo);
        PayEmployeePayDetails.SETFILTER("Pay Element Code",'PF');
