...
{
    DefaultLayout = RDLC;
    RDLCLayout = './Item Don't Have Lead Times.rdlc';

    dataset
    {
        dataitem(Item;Item)
        {
            DataItemTableView = SORTING(No.) ORDER(Ascending) WHERE(Safety Lead Time=FILTER(''),Product Group Code=FILTER(<>FPRODUCT&<>CPCB));
            ...
            begin
                 ...
                 IF NOT "Prod. Order Component".FIND('-') THEN
                 BEGIN
                   sale_Line.RESET;
        ...
        dataitem("PRODUCTION ORDER";"Production Order")
        {
            ...
            trigger OnAfterGetRecord();
            begin
                IF COPYSTR("Sales Order No.",1,7)='EFF/EXP' THEN
                BEGIN
