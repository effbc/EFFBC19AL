report 80015 "Inventory - Availability Plan3"
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './Inventory - Availability Plan3.rdlc';
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING(Field36,Field37) ORDER(Ascending) WHERE(Field2=FILTER(STR|MCH), Field22=CONST(1));
            RequestFilterFields = Field36,Field37;

            trigger OnAfterGetRecord();
            begin
                IF Choice=Choice::Alternate THEN
                BEGIN
                  Change_Alternate_Items(Table50001."No.",Table50001."Prod. Order No.");
                END ELSE IF Choice=Choice::Assign THEN
                BEGIN
                  ...
                  Mat_Issue_sLine.SETRANGE(Mat_Issue_sLine."Document No.",Table50001."No.");
                  ...
                  AssingBatchManual.AssignBatchManual(Table50001);
                
                  //<<Added by Pranavi on 24-Mar-2017
                
                END ELSE IF Choice=Choice::Post THEN
                BEGIN
                /*  Mat_Issue_sLine.RESET;
                  ...
                  Table50001."Posting Date":=TODAY;
                
                  Issue_Post.Issues_Post(Table50001);
                  Mat_Issue_sLine.RESET;
                  Mat_Issue_sLine.SETRANGE(Mat_Issue_sLine."Document No.",Table50001."No.");
                  IF Mat_Issue_sLine.FIND('-') THEN
                  REPEAT
    ...
    begin
          ...
          IF Mat_Issue_sLine.FIND('-') THEN
          REPEAT
            IF Net_Stock(Mat_Issue_sLine."Item No.",Mat_Issue_sLine."Transfer-from Code")<Mat_Issue_sLine."Qty. to Receive" THEN
            BEGIN
              ...
              REPEAT
                IF Net_Stock(Alternate_items."Alternative Item No.",
                            Mat_Issue_sLine."Transfer-from Code")>Mat_Issue_sLine."Qty. to Receive" THEN
                BEGIN
                  Mat_Issue_sLine.TESTFIELD(Mat_Issue_sLine."Quantity Received",0);
                  Mat_Issue_sLine."Item No.":=Alternate_items."Alternative Item No.";
                  IF item.GET(Alternate_items."Alternative Item No.") THEN
                  BEGIN
                    ...
                    Mat_Issue_sLine."Item Category Code" := item."Item Category Code";
                    Mat_Issue_sLine."Product Group Code" := item."Product Group Code";
                    Mat_Issue_sLine.MODIFY;
    ...
    begin
        ...
        MaterialIssuesLine.SETFILTER(MaterialIssuesLine."Prod. Order No.",
                                     Table50001.GETFILTER(Table50001."Prod. Order No."));
        MaterialIssuesLine.SETFILTER(MaterialIssuesLine."Prod. Order Line No.",
                                     Table50001.GETFILTER(Table50001."Prod. Order Line No."));
