...
{
    // version Nirmala,EFFUPG

    DefaultLayout = RDLC;
    RDLCLayout = './Site wise AMC Stock Status.rdlc';
    ...
    dataset
    {
        dataitem("Item Wise Min. Req. Qty at Loc";"Item Wise Min. Req. Qty at Loc")
        {
            ...
            begin
                ...
                IF ("INFORMATION_I/P"="INFORMATION_I/P"::DEFICIET) THEN
                BEGIN
                  IF PLANNED_QTY<="Item Wise Min. Req. Qty at Loc"."Actual Qty" THEN
                     CurrReport.SKIP;
                END ELSE IF ("INFORMATION_I/P"="INFORMATION_I/P"::SURPLUS) THEN
                BEGIN
                  IF PLANNED_QTY>="Item Wise Min. Req. Qty at Loc"."Actual Qty" THEN
                     CurrReport.SKIP;
                END;
                 ITEM_COST:=0;
                 IF ITEM_REC.GET("Item Wise Min. Req. Qty at Loc".Item) THEN
                 BEGIN
                   IF (ITEM_REC."Product Group Code"='CPCB') OR (ITEM_REC."Product Group Code"='FPRODUCT') THEN
                     ITEM_COST:=PRICE_LIST."Calculate Bom Value"("Item Wise Min. Req. Qty at Loc".Item)
                   ELSE IF (ITEM_REC."Product Group Code"='B OUT') THEN
    ...
    var
        ...
        PRICE_LIST : Report "Products Price List";
    ...
    begin
        ...
        REPEAT
          ...
          IF NOT(ITEM_WISE_MIN_REQ.FIND('-')) THEN
          BEGIN
