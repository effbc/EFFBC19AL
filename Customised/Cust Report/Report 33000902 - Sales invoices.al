report 33000902 "Sales invoices"
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './Sales invoices.rdlc';
    ...
    dataset
    {
        dataitem("Sales Invoice Header";"Sales Invoice Header")
        {
            ...
            begin
                PGC:='';
                dlPGC:='';edbPGC:='';ledPGC:='';sftPGC:='';
                ProdPosgroup:='';
                dlProdPosgroup:='';ledProdPosgroup:='';sftProdPosgroup:='';edbProdPosgroup:='';
                cstamt:=0;
                vatamt:=0;
                assessable:=0;
                dlassessable:=0;ledassessable:=0;edbassessable:=0;sftassessable:=0;
                dlcstamt:=0;edbcstamt:=0;ledcstamt:=0;sftcstamt:=0;
                dlvatamt:=0;edbvatamt:=0;ledvatamt:=0;sftvatamt:=0;
                dlQty:=0;sftQty:=0;edbQty:=0;ledQty:=0;
                Excise:=0;
                dlExcise:=0;edbExcise:=0;ledExcise:=0;sftExcise:=0;
                dlAmtToCustomer:=0;edbAmtToCustomer:=0;ledAmtToCustomer:=0;sftAmtToCustomer:=0;
                service:=0;
                actualcess:=0;
                serviceecess:=0;
                instamt:=0;
                  ...
                  BEGIN
                    REPEAT
                      IF (SIL."HSN/SAC Code" <> CurrHSN) THEN BEGIN
                        igst_amt:=0;cgst_amt:=0;sgst_amt:=0;tot_amt_cust:=0;tot_gst_Assessible_amt:=0;gst_qty:=0;
                        SIL1.RESET;
                        ...
                        REPEAT
                          IF SIL1."GST Jurisdiction Type" = SIL1."GST Jurisdiction Type"::Interstate THEN
                            igst_amt+=(SIL1."Total GST Amount")
                          ELSE BEGIN
                            sgst_amt+=(SIL1."Total GST Amount"/2);
                            cgst_amt+=(SIL1."Total GST Amount"/2);
                          END;
                          tot_amt_cust+=SIL1."Amount To Customer";
                          tot_gst_Assessible_amt+=SIL1."Line Amount";
                          gst_qty+=SIL1.Quantity;
                          TCS_Rate := SIL1."TDS/TCS %";
                          TCS_Amount+= SIL1."TDS/TCS Amount";
                        ...
                        ELSE BEGIN
                          ...
                          ELSE IF GLAccnt.GET(SIL."No.") THEN
                        ...
                        IF SIL1."GST Jurisdiction Type" = SIL1."GST Jurisdiction Type"::Interstate THEN
                        BEGIN
                            EnterCell(row,8,FORMAT(ROUND(SIL."GST %",1)),FALSE,TempExcelBuffer."Cell Type"::Number);
                        END ELSE
                        BEGIN
                          EnterCell(row,9,FORMAT(ROUND(SIL."GST %"/2,1)),FALSE,TempExcelBuffer."Cell Type"::Number);
                          EnterCell(row,10,FORMAT(ROUND(SIL."GST %"/2,1)),FALSE,TempExcelBuffer."Cell Type"::Number);
                        END;
                        ...
                        IF user.FIND('-') THEN
                          EnterCell(row,24,FORMAT(user."Full Name"),FALSE,TempExcelBuffer."Cell Type"::Text);
                        IF ("Sales Invoice Header"."Currency Code" <> '') AND ("Sales Invoice Header"."Currency Factor" > 0 ) THEN
                        BEGIN
                        ...
                        IF SIL."Sell-to Customer No." = 'CUST00536' THEN
                        BEGIN
                        ...
                        END ELSE BEGIN
                            ...
                            BEGIN
                                IF CLE."Seller GST Reg. No." <> '' THEN
                                BEGIN
                                    EnterCell(row,27,CLE."Seller GST Reg. No.",FALSE,TempExcelBuffer."Cell Type"::Text);
                                    IF "Sales Invoice Header"."GST Customer Type" =  "Sales Invoice Header"."GST Customer Type"::Unregistered THEN
                                    BEGIN
                        ...
                        BEGIN
                              IF (SIL."Total GST Amount" >0) THEN
                        ...
                        END ELSE IF "Sales Invoice Header"."GST Customer Type" IN ["Sales Invoice Header"."GST Customer Type"::"SEZ Development", "Sales Invoice Header"."GST Customer Type"::"SEZ Unit"] THEN
                        BEGIN
                             IF (SIL."Total GST Amount" >0) THEN
                        ...
                        BEGIN
                              ...
                              BEGIN
                                  ...
                                  BEGIN
                                    IF "Sales Invoice Header"."Customer Posting Group" = 'RAILWAYS' THEN
                                    BEGIN
                        ...
                        END ELSE IF SIL.Type = SIL.Type::"G/L Account" THEN
                        BEGIN
                          GLAct.RESET;
                          ...
                          EnterCell(row,39,FORMAT(SIL.Description),FALSE,TempExcelBuffer."Cell Type"::Text);
                        END;
                      IF  (COPYSTR("Sales Invoice Header"."External Document No.",1,2) = 'SI') OR (COPYSTR("Sales Invoice Header"."External Document No.",1,2) = 'IN') THEN
                      BEGIN
                      ...
                      END;
                      IF (COPYSTR("Sales Invoice Header"."Shortcut Dimension 1 Code",1,3) = 'CUS') THEN
                      BEGIN
                         EnterCell(row,43,'CS',FALSE,TempExcelBuffer."Cell Type"::Text);
                      END ELSE IF (COPYSTR("Sales Invoice Header"."Shortcut Dimension 1 Code",1,3) = 'PRD') THEN
                      BEGIN
                ...
                REPEAT
                  ...
                  BEGIN
                    ...
                    IF SIL."Excise Prod. Posting Group" IN ['84711000','84715000'] THEN
                    BEGIN
                      Item.RESET;
                      Item.SETFILTER(Item."No.",SIL."No.");
                      IF Item.FINDFIRST THEN
                      ...
                      ELSE IF SIL."Tax Area Code"='SALES VAT' THEN
                        dlvatamt:=dlvatamt+SIL."Tax Amount";
                    END;
                    IF SIL."Excise Prod. Posting Group"='85414020' THEN
                    BEGIN
                      Item.RESET;
                      Item.SETFILTER(Item."No.",SIL."No.");
                      IF Item.FINDFIRST THEN
                    ...
                    END;
                    IF SIL."Excise Prod. Posting Group"='90221900' THEN
                    BEGIN
                      Item.RESET;
                      Item.SETFILTER(Item."No.",SIL."No.");
                      IF Item.FINDFIRST THEN
                    ...
                    END;
                    IF SIL."Excise Prod. Posting Group" IN['94054090','LED_MRP'] THEN
                    BEGIN
                      Item.RESET;
                      Item.SETFILTER(Item."No.",SIL."No.");
                      IF Item.FINDFIRST THEN
                  ...
                  BEGIN
                    ...
                    IF ((SIL."No."='46400') OR(SIL."No."='47505')) AND (SIL."Amount To Customer">0) THEN
                    BEGIN
                      typeofservice:='Software';
                      softamt:=softamt+SIL.Amount;
                    END;
                    IF (SIL."No."='47300') AND (SIL."Amount To Customer">0) THEN
                    BEGIN
                      typeofservice:='Installation';
                  ...
                  END;
                  IF pos1<>0 THEN
                  IF (SIL."No."='47503') AND (SIL."Amount To Customer">0) THEN
                  BEGIN
                ...
                */
                  ...
                  //IF normal THEN BEGIN
                    ...
                    //IF "Sales Invoice Header"."Total Invoiced Amount"<>0 THEN BEGIN // REV01 >>
                      ...
                      BEGIN
                        ...
                        EnterCell(row,20,FORMAT("Sales Invoice Header"."Form Code"),FALSE,TempExcelBuffer."Cell Type"::Text);
                        ...
                        user.SETFILTER(user.EmployeeID,"Sales Invoice Header"."Salesperson Code");
                        IF user.FIND('-') THEN
                          EnterCell(row,23,FORMAT(user."Full Name"),FALSE,TempExcelBuffer."Cell Type"::Text);
                      ...
                      BEGIN
                        ...
                        ELSE
                          EnterCell(row,26,'11111111111',FALSE,TempExcelBuffer."Cell Type"::Text);
                      END;
                      IF (dlQty = 0) AND (edbQty = 0) AND (ledQty = 0) AND (sftQty = 0) AND (assessable >0) THEN
                      BEGIN
                        row:=row+1;
                        EnterCell(row,1,FORMAT(row-1),FALSE,TempExcelBuffer."Cell Type"::Number);
                        EnterCell(row,2,FORMAT("Sales Invoice Header"."Posting Date"),FALSE,TempExcelBuffer."Cell Type"::Date);
                ...
                IF serviceinv THEN
                BEGIN
                  IF "Sales Invoice Header"."Total Invoiced Amount"<>0 THEN
                  BEGIN
                    row:=row+1;
                    EnterCell(row,1,FORMAT(row-1),FALSE,TempExcelBuffer."Cell Type"::Number);
                    EnterCell(row,2,FORMAT("Sales Invoice Header"."Posting Date"),FALSE,TempExcelBuffer."Cell Type"::Date);
                    ...
                    user.SETFILTER(user.EmployeeID,"Sales Invoice Header"."Salesperson Code");
                    IF user.FIND('-') THEN
                      EnterCell(row,20,FORMAT(user."Full Name"),FALSE,TempExcelBuffer."Cell Type"::Text);
                    // $Pranavi End
                    Customer.RESET;
                    Customer.SETFILTER(Customer."No.","Sales Invoice Header"."Sell-to Customer No.");
                    IF Customer.FINDFIRST THEN
        ...
        layout
        {
            area(content)
            {
                group(General)
                {
                    ...
                    field("Note :  1. If you Want Only Service Bills With Excel Please Check Service Invoices & Excel";'')
                    {
                        Caption = 'Note :  1. If you Want Only Service Bills With Excel Please Check Service Invoices & Excel';
                        Style = Strong;
                        StyleExpr = TRUE;
                    }
                    field(Control1102152006;'')
                    {
