...
{
    // version DIM1.0,Rev01,Eff02,EFFUPG
                ...
                */
                ...
                IF DimSetEntryGRec.FINDSET THEN
                  REPEAT
                    ...
                    END;
                    IF DimSetEntryGRec."Dimension Code" <> 'DEPARTMENTS' THEN
                    BEGIN
                      DimSetEntryGRec.CALCFIELDS("Dimension Value Name");
                ...
                CASE Acount_No OF
                  ...
                  20200..20900 :
                    BEGIN
                        IF "G/L Entry"."Sale Order No." <> '' THEN
                        BEGIN
                ...
                BEGIN
                  Entry_Type := 'Sales';
                END ELSE IF PIH.FINDFIRST THEN
                BEGIN
                    Entry_Type := 'Purchase';
                END ELSE IF PCN.FINDFIRST THEN
                BEGIN
                  Entry_Type := 'Credit Note';
                END ELSE IF  PDN.FINDFIRST THEN
                BEGIN
                  Entry_Type := 'Debit Note';
                END ELSE IF  (COPYSTR("G/L Entry"."Document No.",1,3)='BPV') OR (COPYSTR("G/L Entry"."Document No.",1,3)= 'CPV') THEN
                BEGIN
                  Entry_Type := 'Payment';
                END
                ELSE IF  (COPYSTR("G/L Entry"."Document No.",1,3)='CRV') OR (COPYSTR("G/L Entry"."Document No.",1,3)= 'BRV') THEN
                BEGIN
                    Entry_Type := 'Reciept';
                END
                ELSE IF  (COPYSTR("G/L Entry"."Document No.",1,2)='JV') THEN
                BEGIN
                    Entry_Type := 'JV';
                END;
