        ...
        dataitem("Sales Header";"Sales Header")
        {
            ...
            dataitem("Sales Line";"Sales Line")
            {
                ...
                begin
                    ...
                    ("Sales Line"."Tax Group Code"='2%')) THEN
                    BEGIN
                    ...
                    IF (Type = Type::"G/L Account") AND (NOT ShowInternalInfo) THEN
                      "No." := '';
                     //SREENIVAS -EFF
                    IF "Sales Line"."Tax Group Code"='MPBI' THEN
                    BEGIN
                    s:='84711000';
                    capt:='(Microprocessor based data acquisition&control system)';
                    END;
                    IF "Sales Line"."Tax Group Code"='EDB' THEN
                    BEGIN
                    s:='85414020';
                    ...
                    BEGIN
                      IF (("Sales Line"."Tax Group Code"='BOI-COMPUT') OR ("Sales Line"."Tax Group Code"='BOI-O') OR (
                         "Sales Line"."Tax Group Code"='BOI-O 2%') OR  ("Sales Line"."Tax Group Code"='RAW') OR (
                         "Sales Line"."Tax Group Code"='RAW 10.5') OR ("Sales Line"."Tax Group Code"='RAW 12.5') OR
                         ("Sales Line"."Tax Group Code"='SOFTWARE') OR ("Sales Line"."Tax Group Code"='2%')) THEN
                       BEGIN
                        bedamt:=0;
                        cessamt:=0;
                        ecessamt:=0;
                       ...
                       END;
                      IF (("Sales Header".Structure='SALE-E+CST')OR ("Sales Header".Structure='SALE-CST')) AND("Sales Line"."Tax %"=2) THEN
                      BEGIN
                      ...
                      IF ("Sales Line"."Tax %"=0) THEN
                        caplabel:='';
                      IF (("Sales Header".Structure='SALE-E+CST')OR ("Sales Header".Structure='SALE-CST')) AND("Sales Line"."Tax %"=3) THEN
                      BEGIN
                        caplabel:='(3% aganist Form - C)';
                       taxamt:=(("Sales Line"."Unit Price"*"Sales Line"."Qty. to Ship")+bedamt+cessamt+ecessamt)*0.03;
                       END;
                      IF (("Sales Header".Structure='SALE-E+CST')OR ("Sales Header".Structure='SALE-CST')) AND ("Sales Line"."Tax %"=12.5) THEN
                      BEGIN
                        caplabel:='(12.5%)';
                         taxamt:=(("Sales Line"."Unit Price"*"Sales Line"."Qty. to Ship")+bedamt+cessamt+ecessamt)*0.125;
                      END;
                      IF (("Sales Header".Structure='SALE-E+V')OR ("Sales Header".Structure='SALE-VAT')) AND ("Sales Line"."VAT %"=4) THEN
                      BEGIN
                        caplabel1:='4%';
                       vatamt:=(("Sales Line"."Unit Price"*"Sales Line"."Qty. to Ship")+bedamt+cessamt+ecessamt)*0.04;
                       END;
                      IF (("Sales Header".Structure='SALE-E+V')OR ("Sales Header".Structure='SALE-VAT'))AND("Sales Line"."VAT %"=12.5) THEN
                      BEGIN
                      caplabel1:='12.5%';
