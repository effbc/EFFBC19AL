page 60183 "Purchase Order-Inventory"
{
    ...
    SourceTableView = WHERE(Document Type=FILTER(Order),Subcontracting=CONST(No));
            ...
            group("P&osting")
            {
                ...
                action("P&ost")
                {
                    ...
                    begin
                        ...
                        PurchLine.SETRANGE(PurchLine."Document No.","No.");
                        PurchLine.SETFILTER(PurchLine."Qty. to Receive",'>%1',0);
                        PurchLine.SETRANGE(PurchLine."Product Group Code",'PCB');
                        PurchLine.SETCURRENTKEY("Document Type",Type,"No.");
                        IF PurchLine.FINDSET THEN
                        REPEAT
                            IF  PurchLine."No." =Purch_Line_Itm_No THEN
                              BEGIN
                              END
                            ELSE
                              BEGIN
