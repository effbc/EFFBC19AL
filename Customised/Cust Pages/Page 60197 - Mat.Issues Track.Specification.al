page 60197 "Mat.Issues Track.Specification"
{
            ...
            repeater(Control1000000000)
            {
                ...
                field("Serial No.";"Serial No.")
                {
                    ...
                    begin
                        ...
                        AssistedditLotSerialNo(Rec,0,"UndefinedQty.","Order No.");
                        CurrPage.UPDATE;
                    end;
                }
                field("Lot No.";"Lot No.")
                {
                    ...
                    begin
                        ...
                        IF "UndefinedQty." = 0 THEN
                          MaterialIssueLineReserve.AssistEditLotSerialNo(Rec,1,Quantity,"Order No.")
                        ELSE
                          MaterialIssueLineReserve.AssistEditLotSerialNo(Rec,1,"UndefinedQty.","Order No.");
                        CurrPage.UPDATE;
                    end;
                }
                field(QuantityN;Quantity)
                {
                    Caption = 'Quantity';
    ...
    var
        ...
        "RemQty." : Decimal;
    begin
        MaterialIssueLine.SETRANGE("Document No.","Order No.");
        MaterialIssueLine.SETRANGE("Line No.","Order Line No.");
        IF MaterialIssueLine.FINDFIRST THEN
          SourceQuantityArray[1] := MaterialIssueLine."Qty. to Receive"
        ELSE
        BEGIN
        DCL.SETRANGE("Document No.","Order No.");
        DCL.SETRANGE("Line No.","Order Line No.");
    ...
    begin
        // Rev01 >>
        MaterialIssueLine.SETRANGE("Document No.",GETFILTER("Order No."));
        MaterialIssueLine.SETFILTER("Line No.",GETFILTER("Order Line No."));
        IF MaterialIssueLine.FINDFIRST THEN
          SourceQuantityArray[1] := MaterialIssueLine."Qty. to Receive"
        ELSE
        BEGIN
        DCL.SETRANGE("Document No.",GETFILTER("Order No."));
        DCL.SETFILTER("Line No.",GETFILTER("Order Line No."));
        IF DCL.FINDFIRST THEN
          SourceQuantityArray[1] := DCL.Quantity;
        END;

        UpdateUndefinedQty;
        GetItem(FORMAT(GETFILTER("Item No.")));
        UndefinedQtyOnFormat;
    ...
    var
        TrackingSpecification : Record "Mat.Issue Track. Specification";
    begin
        TrackingSpecification.SETRANGE("Order No.",GETFILTER("Order No."));
        TrackingSpecification.SETFILTER("Order Line No.",GETFILTER("Order Line No."));
    ...
    begin
        ...
        IF ItemLedgEntry.FINDSET THEN REPEAT
          ...
          IsExp:=FALSE;
          IF (ItemLedgEntry."Location Code" IN ['CS STR','R&D STR','PRODSTR','STR','MCH']) THEN
          BEGIN
          ...
          BEGIN
          CASE LookupMode OF
            LookupMode::"Serial No.": TempEntrySummary.SETRANGE("Serial No.",ItemLedgEntry."Serial No.");
            LookupMode::"Lot No.": TempEntrySummary.SETRANGE("Lot No.",ItemLedgEntry."Lot No.");
          END;
        /*  TempEntrySummary.INIT;
          ...
          */
          ...
          // End--Pranavi
            ...
            END ELSE
            BEGIN
              IF (ItemLedgEntry.Quantity-QLE."Remaining Quantity")>0 THEN
              BEGIN
                TempEntrySummary.INIT;
                TempEntrySummary."Entry No." := ItemLedgEntry."Entry No.";
                TempEntrySummary."Table ID" := DATABASE :: "Material Issues Line";
                ...
                TempEntrySummary.USERID := UsrId;    // Pranavi
               IF NOT (TempEntrySummary."Total Quantity"=0)THEN
               BEGIN
                 TempEntrySummary.INSERT;
                 IF ItemLedgEntry.Positive THEN
                 BEGIN
        ...
        IF ItemTrackingSummaryForm.RUNMODAL = ACTION::LookupOK THEN BEGIN
          ...
          // Added by Pranavi on 26-Nov-2016 for Restrincting Return Material to only select req. person material only
          IF NOT( TrackingSpecification."Location Code" IN ['SITE','CS STR','R&D STR','CS','PRODSTR','STR','MCH','PROD']) THEN
          BEGIN
            MIH.RESET;
          ...
          IF Item.GET(TrackingSpecification."Item No.") THEN
            TrackingSpecification."Product Group Code":=Item."Product Group Code";
          IF "Material Issues Header".GET("IssueNo.") THEN
          BEGIN
