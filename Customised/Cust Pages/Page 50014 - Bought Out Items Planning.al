        ...
        area(content)
        {
            ...
            repeater(Group)
            {
                ...
                field("Planned Dispatch Date";"Planned Dispatch Date")
                {
                    ...
                    trigger OnValidate();
                    begin
                        IF USERID IN ['EFFTRONICS\PRANAVI','EFFTRONICS\VIJAYA','EFFTRONICS\ANILKUMAR','EFFTRONICS\BSATISH','EFFTRONICS\SARDHAR','EFFTRONICS\SPURTHI','EFFTRONICS\ANVESH'] THEN
                        BEGIN
                ...
                field(To_Be_Purch_Qty;To_Be_Purch_Qty)
                {
                    ...
                    trigger OnValidate();
                    begin
                        IF NOT (USERID IN ['EFFTRONICS\PRANAVI','EFFTRONICS\VIJAYA','EFFTRONICS\ANILKUMAR','EFFTRONICS\CHOWDARY','EFFTRONICS\BRAHMAIAH','EFFTRONICS\RENUKACH','EFFTRONICS\ANANDA','EFFTRONICS\PARDHU']) THEN
                        BEGIN
                          MESSAGE('You do not have rights for Planning!');
                          ERROR('You do not have rights for Planning!');
                        END;
                        ...
                        IF To_Be_Purch_Qty > 0 THEN
                          IF To_Be_Purch_Qty > "To Be Shipped Qty" THEN
                          BEGIN
        ...
        area(processing)
        {
            ...
            action("Show Spec")
            {
                ...
                begin
                    ...
                    ELSE IF "Schedule Line No" > 0 THEN
            ...
            action("Create Orders")
            {
                ...
                var
                    ...
                    SCh2Filter : Record Schedule2;
                begin
                    IF USERID IN ['EFFTRONICS\PRANAVI','EFFTRONICS\VIJAYA','EFFTRONICS\ANILKUMAR','EFFTRONICS\CHOWDARY','EFFTRONICS\BRAHMAIAH','EFFTRONICS\RENUKACH','EFFTRONICS\ANANDA','EFFTRONICS\PARDHU'] THEN
                    BEGIN
                      SETCURRENTKEY("Asign Vendor","Document Type","Document No",No,"Line No","Schedule Line No");
                      SETFILTER("Asign Vendor",'<>%1','');
                      IF FINDSET THEN
                      REPEAT
                        IF Previous_Vendor<>"Asign Vendor" THEN
                        BEGIN
                          Previous_Vendor:="Asign Vendor";
                          Vendor.RESET;
                          IF Vendor.GET("Asign Vendor") THEN
                          BEGIN
                      ...
                      Previous_Vendor:=''; Previous_Order := '';Previous_Item:='';
                      "No. Of Order":=0;
                      MinDate := 0; MaxDate := 0;
                      Count := FALSE; Count1 := FALSE;
                      Orders_List := '';
                      ...
                      IF FINDSET THEN
                      REPEAT
                        IF (Previous_Order <> "Document No") THEN
                        BEGIN
                          ...
                          BEGIN
                            ...
                            BEGIN
                              IF PurchaseHeader."Requested Receipt Date" > (TODAY+(MinDate -3)) THEN
                              BEGIN
                            ...
                            BEGIN
                              IF PurchaseHeader."Expected Receipt Date" < (TODAY+MaxDate) THEN
                              BEGIN
                            ...
                            BEGIN
                              ...
                              REPEAT
                                ...
                                BEGIN
                                  IF SCh2."No." = '' THEN
                                    S_LineNo:=S_LineNo+DELCHR(FORMAT(SCh2."Line No."),'=',',')+'|'
                                  ELSE IF (SCh2."Document Line No." = SCh2."Line No.") OR ("Schedule Line No" = SCh2."Line No.") THEN
                                  BEGIN
                                  END ELSE loopbreak := TRUE;
                                END;
                                LoopCount:=LoopCount-1;
                                IF LoopCount < 2 THEN
                                  loopbreak := TRUE;
