        ...
        area(content)
        {
            ...
            repeater(Group)
            {
                field("SD Fin Verification";"SD Fin Verification")
                {
                    ...
                    trigger OnValidate();
                    begin
                        IF NOT (USERID IN['EFFTRONICS\RAJANI','EFFTRONICS\VISHNUPRIYA','EFFTRONICS\SUSMITHAL']) THEN
                          BEGIN
                              ERROR('You Do Not Have a rights');
                          END;
                        IF "Consignee Name" =  'New Running Order' THEN
                          BEGIN
                            SH.RESET;
    ...
    begin
        ...
        BEGIN
          REPEAT
            ...
            IF SalesInvHdr."No." = LastOrder THEN
            BEGIN
              Rec := SalesInvHdr;
              INSERT;
              IF COPYSTR("Order No.",5,3) = 'AMC' THEN
              BEGIN
                Customer.RESET;
