page 50016 "MSL Items List"
{
            ...
            repeater(Group)
            {
                ...
                field("Baked Hours";"Baked Hours")
                {
                    ...
                    begin
                        ...
                        BEGIN
                          ...
                          BEGIN
                            IF (Itm."Component Shelf Life(Years)" > 0) AND ((DateAndTime.DateDiff('YYYY',"MFD Date",TODAY,DayofWeekInput,WeekofYearInput) >= Itm."Component Shelf Life(Years)"))   THEN
                              ERROR('You cannot bake already Shelf Life Expired Items!');
                            IF "Recharge Cycles" >= 2 THEN
                            ...
                            BEGIN
                              ...
                              IF Itm_floor_life > "Floor Life" THEN
                                ERROR('%1 hours of floor life still remaining. You cannot bake until then!',Itm_floor_life - "Floor Life");
                            END;
                ...
                field(Open;Open)
                {
                    ...
                    begin
                        IF Open THEN
                        BEGIN
                          IF NOT ("Global Dimension 1 Code" IN ['S_Expired','R_Expired','S_EXPIRED','R_EXPIRED']) THEN
                          BEGIN
                            Open:=FALSE;
        ...
        area(processing)
        {
            ...
            action("Update Bake Hours")
            {
                ...
                begin
                      ...
                      BEGIN
                       REPEAT
                        IF ILE.GET("Entry No.") THEN
                        BEGIN
                          ILE."Floor Life":=0;
                          ILE."Recharge Cycles"+=1;
                          ILE."MBB Packed Date":="MBB Packed Date";
                          ILE.MODIFY;
                        END;
                      UNTIL NEXT=0;
                      END;
                        ...
                        BEGIN
                          ...
                          BEGIN
                            ...
                            BEGIN
                              ...
                              AssignTrackingSpec(MaterialIssueLine,"Lot No.");
                            END;
                          END;
    ...
    begin
        ...
        BEGIN
          ...
          MSL:=Itm.MSL;
          ESD:=Itm."ESD Class";
          IF (Itm."Component Shelf Life(Years)" > 0) AND ((DateAndTime.DateDiff('M',"MFD Date",TODAY,DayofWeekInput,WeekofYearInput) >= Itm."Component Shelf Life(Years)"*12))   THEN
            ColorCode := 'Ambiguous';
          IF ColorCode = '' THEN
          BEGIN
            IF "Recharge Cycles" >= 2 THEN
            ...
            IF ColorCode = '' THEN
              ...
              BEGIN
                ...
                IF ColorCode = '' THEN
                BEGIN
                  IF "Floor Life" >= ROUND(Itm_floor_life*90/100,1,'>') THEN
                    ColorCode := 'StrongAccent';
                END;
              END;
              IF ColorCode = '' THEN
              BEGIN
                IF (Itm."Component Shelf Life(Years)" > 0) AND (Itm."Component Shelf Life(Years)"*12 - DateAndTime.DateDiff('M',"MFD Date",TODAY,DayofWeekInput,WeekofYearInput) <= 1) THEN
                  ColorCode := 'Favorable';
              END;
          END;
          "External Document No." := ColorCode;
          MODIFY;
        END;
    end;
    ...
    begin
        ...
        BEGIN
          IF Itm.GET(FORMAT(RecordSet.Fields.Item('No_').Value)) THEN
          BEGIN
            ILE.RESET;
            IF ILE.GET(FORMAT(RecordSet.Fields.Item('Purch_Rcpt_Entry_No').Value)) THEN
            BEGIN
              Rec.INIT;
              Rec := ILE;
    ...
    begin
        ...
        IF ITEM1.GET(MILin."Item No.") THEN
          TrackingSpecification."Product Group Code":=ITEM1."Product Group Code";

        IF MIH.GET(MILin."Document No.") THEN
        BEGIN
    ...
    begin
        ...
        BEGIN
          ...
          SETFILTER("Global Dimension 1 Code",FilterCodes);
        END;
        /*
        RESET;
        IF FINDSET THEN
