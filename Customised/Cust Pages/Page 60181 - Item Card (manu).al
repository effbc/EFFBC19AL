        ...
        area(content)
        {
            group(General)
            {
                ...
                field("No.";"No.")
                {
                    ...
                    trigger OnValidate();
                    begin
                        IF USERID IN ['EFFTRONICS\PADMASRI','EFFTRONICS\MARY'] THEN
                        BEGIN
                ...
                field("Item Category Code";"Item Category Code")
                {
                    ...
                    trigger OnValidate();
                    begin
                        IF "User ID" IN ['EFFTRONICS\PADMASRI','EFFTRONICS\MARY'] THEN
                        BEGIN
                ...
                field("Product Group Code";"Product Group Code")
                {
                    ...
                    trigger OnValidate();
                    begin
                        IF "Item Category Code" = 'OFFICE MAI' THEN
                        BEGIN
                            IF "Product Group Code" = 'CANTEEN' THEN
                            BEGIN
                                "Tax Group Code" := 'FOOD &BEV';
                                "Spec ID" := 'I-STA-0001';
                            END
                            ELSE IF ("Product Group Code" IN [ 'ELE','HKE','MISC','STATIONARY'] )THEN
                            BEGIN
                         ...
                         IF xcnt =2 THEN
                           BEGIN
                             Subject := Body;
                                ...
                                IF NOT(FORMAT(t1[i])  IN ['0'..'9']) THEN ERROR('USE NUMBERS AND X FOR THE SEPARATOR LIKE 10X200X25.AVOID SPACES IN PART1'+t1) END;
                                ...
                                IF NOT(FORMAT(t2[i])  IN ['0'..'9']) THEN ERROR('USE NUMBERS AND X FOR THE SEPARATOR LIKE 10X200X25.AVOID SPACES IN PART2'+t2) END;
                                ...
                                IF NOT(FORMAT(t3[i])  IN ['0'..'9']) THEN ERROR('USE NUMBERS AND X FOR THE SEPARATOR LIKE 10X200X25.AVOID SPACES IN PART3'+t3) END;

                           END
                           ELSE IF STRLEN(Body) > 0 THEN
