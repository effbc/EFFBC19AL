page 50009 "Purchase Automail VendorsList"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field("Send Mail";"Send Mail")
                {
                }
                field("Mail Required";"Mail Required")
                {
                    Visible = false;
                }
            }
                ...
                action("Send Auto Mail")
                {
                    ...
                    begin
                        ...
                        */
                        ...
                        IF ( Mail_Purpose = 'Material') THEN
                        BEGIN// end by vijaya
                        IF FINDSET THEN
                        REPEAT
                          first := FALSE;
                          Mail_To :="E-Mail"; // 'rakesht@efftronics.com';
                          PL.RESET;
                          PL.SETCURRENTKEY(PL."Document Type",PL."Buy-from Vendor No.");
                          PL.SETFILTER(PL."Document Type",'%1',1);
                          PL.SETFILTER(PL."Qty. to Receive",'>%1',0);
                          ...
                          REPEAT
                            ...
                            BEGIN     //End By Pranavi on 26-Dec-2015 to send mail only for released order items
                              IF first = FALSE THEN
                              BEGIN
                                Mail_Subject := 'Reg: Pending Material Expected dates Required -'+ Name;
                                Mail_Body := '';
                                ...
                                SMTP_MAIL.AppendBody('<div style="Background-color:#0971D9; color:#F0F1F5; "><br/><label><font size="6">Material Expected Dates Required</font></label></div><br/>');
                                SMTP_MAIL.AppendBody('Dear Sir/Madam,');
                                SMTP_MAIL.AppendBody('<br><br>');
                                SMTP_MAIL.AppendBody('Please provide the material expected date for the below pending material.<br>');
                                SMTP_MAIL.AppendBody('Please mention the dispatch date in below field and reply through return mail.<br><br><HR COLOR=#26148C>');
                        
                                SMTP_MAIL.AppendBody('<table border=1 style="border-collapse: collapse;font-size:10pt; width:100%;">');
                                SMTP_MAIL.AppendBody('<tr><th>Purchase Order No.</th><th>Order Date</th><th>Description</th><th>Qty. to Receive</th><th>Pending Days</th><th>Exp. Dispatch Date</th></tr>');
                                PurchaseHeader.RESET;
                                PurchaseHeader.SETRANGE(PurchaseHeader."No.",PL."Document No.");
                                IF PurchaseHeader.FINDFIRST THEN
                                BEGIN
                                ...
                                ELSE*/
                                  font_color := 'black';
                                SMTP_MAIL.AppendBody('<tr style="color:'+font_color+';"><td>'+PL."Document No."+'</td><td>'+FORMAT(Ord_date)+'</td>');
                                SMTP_MAIL.AppendBody('<td>'+PL.Description+'</td><td align=right>'+FORMAT(PL."Qty. to Receive")+'</td><td align=right>'+FORMAT(TODAY-Ord_date)+'</td><td align=center>-</td></tr>');
                                first := TRUE;
                              END
                              ELSE
                              BEGIN
                          ...
                          BEGIN
                            ...
                            SMTP_MAIL.AppendBody('<br><br><HR COLOR=#26148C><br>Regards,<br><b>'+username+'<br> Purchase Department<BR>'+'Efftronics Systems Pvt. Ltd.,</b><BR>');
                            SMTP_MAIL.AppendBody('40-15-9,Brundavan Colony,<br>Vijayawada - 520010,<br>Andhra Pradesh, India.<br>');
                            SMTP_MAIL.AppendBody('Ph No : 0866-2466679(Direct)/2466699/75<br>');
                            ...
                            END
                          ELSE
                            MESSAGE('Mail-ID N/A for '+Name);
                          END;
                        UNTIL NEXT=0;
                        END
                        //Added by vijaya on 24-May-2016 for Differentiate Material Receive and Bill Receive
                        ELSE IF Mail_Purpose = 'Bill' THEN
                        BEGIN
                          IF FINDSET THEN
                          REPEAT
                              first := FALSE;
                             ...
                             //Mail_To := 'anilkumar@efftronics.com,pranavi@efftronics.com,vijaya@efftronics.com';
                              PRH.RESET;
                              PRH.SETFILTER(PRH."Buy-from Vendor No.","No.");
                              ...
                              REPEAT
                                  ...
                                  REPEAT
                                      IF first = FALSE THEN
                                      BEGIN
                                        Mail_Subject := 'Reg: Original invoice  Required -'+ Name;
                                        Mail_Body := '';
                                        SMTP_MAIL.CreateMessage('ERP',Mail_From,Mail_To,Mail_Subject,Mail_Body,TRUE);
                                        SMTP_MAIL.AppendBody('<html><body><div style="border-color:white;  margin: 20px; border-width:10px; font-size:11pt;  border-style:solid; padding: 20px; width: 800px;">');
                                        SMTP_MAIL.AppendBody('<div style="Background-color:#0971D9; color:#F0F1F5; "><br/><label><font size="6">Original invoice  Required</font></label></div><br/>');
                                        SMTP_MAIL.AppendBody('Dear Sir/Madam,');
                                        SMTP_MAIL.AppendBody('<br><br>');
                                        SMTP_MAIL.AppendBody('Below list is already material received and Original invoice to be received report.<br>');
                                        SMTP_MAIL.AppendBody('Please courier original invoice and please mention expected courier date & Docket details.<br><br><HR COLOR=#26148C>');
                        
                                        SMTP_MAIL.AppendBody('<table border=1 style="border-collapse: collapse;font-size:10pt; width:100%;">');
                                        SMTP_MAIL.AppendBody('<tr><th>Purchase Order No.</th><th><Description></th><th>Quantity</th><th>Amount</th><th>Received Date</th>');
                                        SMTP_MAIL.AppendBody('<th>Pending Days</th><th>Exp. Couriour date & Docket details</th></tr>');
                                        SMTP_MAIL.AppendBody('<tr><td>'+PRH."Order No."+'</td><td>'+PRL.Description+'</td><td align=right>'+FORMAT(PRL.Quantity)+'</td><td align=right>');
                                        SMTP_MAIL.AppendBody(FORMAT(PRL.Quantity*PRL."Unit Cost (LCY)")+'</td><td>'+FORMAT(PRH."Posting Date") +'</td><td align=right>');
                                        SMTP_MAIL.AppendBody(FORMAT(TODAY -PRH."Posting Date") +'</td><td  align=center>-</td></tr>');
                                          first := TRUE;
                                      END
                                      ELSE
                                      BEGIN
    ...
    begin
        RESET;
        MODIFYALL("Send Mail",FALSE);

        RESET;
        SETRANGE("Mail Required",TRUE);
    end;
