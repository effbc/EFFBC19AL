            ...
            group(Invoicing)
            {
                ...
                field("Payment Terms Code";"Payment Terms Code")
                {
                    ...
                    begin
                        ...
                        IF (xRec."Payment Terms Code" <> "Payment Terms Code") AND (xRec."Payment Terms Code" <> '') THEN
                        BEGIN
                          ...
                          IF PayTerm.FINDFIRST THEN
                          BEGIN
                            IF PayTerm."Stage 1" = PayTerm."Stage 1"::Advance THEN
                            BEGIN
                              CustLedgEntr.RESET;
            ...
            group("Request Approval")
            {
                ...
                action("Re&lease")
                {
                    ...
                    begin
                        ...
                        REPEAT
                        IF SCHEDULEOMS.Type=SCHEDULEOMS.Type::Item THEN
                        BEGIN
                        ...
                        IF COPYSTR("No.",14,2) IN ['/T','/L'] THEN
                        BEGIN
                        ...
                        IF ("Customer Posting Group" IN ['PRIVATE','OTHERS','EXPORT']) AND NOT ("Sell-to Customer No." IN['CUST00536','CUST01164']) AND (Order_After_CF_Integration = TRUE) THEN
                        BEGIN
                          CUS.RESET;
                          CUS.SETRANGE(CUS."No.","Sell-to Customer No.");
                          IF CUS.FINDFIRST THEN
                          BEGIN
                            IF CUS."Customer Posting Group" IN['PRIVATE','OTHERS','EXPORT'] THEN
                            BEGIN
                              PT.RESET;
                              ...
                              IF PT.FINDFIRST THEN
                                IF (PT."Stage 1"=PT."Stage 1"::Credit) OR (PT."Stage 2"=PT."Stage 2"::Credit) OR (PT."Stage 3"=PT."Stage 3"::Credit) THEN
                                  ...
                                  ELSE IF CUS."Payment Term Auth" IN[CUS."Payment Term Auth"::"Sent For Authorization",CUS."Payment Term Auth"::" "] THEN
                          ...
                          BEGIN
                            ...
                            BEGIN
                              ...
                              BEGIN
                                ...
                                MODIFY;
                              END
                              ELSE ERROR('Please enter Payment terms code in customer card!');
                            END;
                          END
                          ELSE
                          BEGIN
                              ...
                              BEGIN
                                ...
                                REPEAT
                                  ...
                                  IF SalesLine.Type = SalesLine.Type::Item THEN BEGIN
                                    ...
                                    END;
                                  END
                                  ELSE IF SalesLine.Type = SalesLine.Type::"G/L Account" THEN BEGIN
                                ...
                                IF (PaymentTerms."Stage 1"=PaymentTerms."Stage 1"::Credit) OR (PaymentTerms."Stage 1"=PaymentTerms."Stage 2"::Credit) OR
                                   (PaymentTerms."Stage 1"=PaymentTerms."Stage 3"::Credit) THEN
                                BEGIN
                                  CUS.RESET;
                                  CUS.SETRANGE(CUS."No.","Sell-to Customer No.");
                                  IF CUS.FINDFIRST THEN
                                ...
                                ELSE BEGIN
                                  IF (PaymentTerms."Stage 1" = PaymentTerms."Stage 1"::Advance) AND (PaymentTerms."Percentage 1" <> 0) THEN
                                  BEGIN
                                    CLE.RESET;
                                    ...
                                    BEGIN
                                      REPEAT
                                        ...
                                        UNTIL DCLE.NEXT = 0;
                                      UNTIL CLE.NEXT = 0;
                                      IF ABS(ROUND(AdvPaymentAmt,0.01,'>') - ROUND((SupplyAmt*PaymentTerms."Percentage 1")/100,0.01,'>')) > 1 THEN
                                      BEGIN
                                        ...
                                        ELSE IF "PT Release Auth Stutus" = "PT Release Auth Stutus"::Rejected THEN
                                          ERROR('Your permission for releasing is rejected!');
                                        IF "PT Release Auth Stutus" = "PT Release Auth Stutus"::" " THEN
                                        BEGIN
                                ...
                                END;
                              END
                              ELSE ERROR('Payment Terms Code '+"Payment Terms Code"+' does not exist in Payment Terms Table!');
                            END;
                          END;
                        END;
                        ...
                        BEGIN
                          ...
                          IF NOT ("Sell-to Customer No." IN ['CUST00536','CUST01164']) THEN
                          BEGIN
                    ...
                    begin
                        ...
                        // end by Rakesh
                         ...
                         ERROR('Posting Date must be Greater than Order Date');
                          ...
                          IF InvoiceNos IN [InvoiceNos::ExciseInv,InvoiceNos::InstInv,InvoiceNos::TradingInv] THEN
                          BEGIN
                            IF DepartmentsCode <> 'PRD' THEN
                              ERROR('Departments Code must be "PRD" for %1',InvoiceNos);
                          END;
                          IF InvoiceNos IN [InvoiceNos::ServiceInv] THEN
                          BEGIN
                        ...
                        IF POSTEDINVOICE1.FINDFIRST THEN
                        BEGIN
                            ...
                            IF SalesLine.FINDSET THEN BEGIN
                              REPEAT
                                  ...
                                  IF ScheduleComp.FINDSET THEN BEGIN
                                          REPEAT
                                          IF ScheduleComp."Document Line No." <> ScheduleComp."Line No." THEN
                                          BEGIN
                ...
                begin
                    ...
                    WHILE NOT RecordSet.EOF DO
                      BEGIN
                        ...
                        IF ("No."=FORMAT(RecordSet.Fields.Item('orderno').Value))  THEN
                        BEGIN
                          IF "No."<>'' THEN
                          BEGIN
                          //Authorized:=TRUE;
                     ...
                     // SalesLine.SETRANGE(Type,SalesLine.Type::Item);
                      ...
                      BEGIN
                        // MESSAGE('Enter in OMS Part');      OR ("Document Type"="Document Type"::"Blanket Order")
                         ...
                         BEGIN
                           IF (OMSIntegrateCode.SaleOrderCreationinOMS(Rec)) =FALSE THEN
                           BEGIN
                             MESSAGE('Error occured in OMS integration and record is not posted');
                           END;
                         END;
                      END;
                    END ELSE ERROR('Order in Open State');
