table 33000026 "Pay Employee Time Sheet Sub"
{
    ...
    begin
        ...
        if not PayEmployeeTimeSheetMain.Find('-') then begin
            EmployeeMaster.SetRange(EmployeeMaster."No.", "Employee No");
            if EmployeeMaster.Find('-') then begin
    ...
    begin
        ...
        if TimeSheetSub.Find('+') then
            if "Time From" < TimeSheetSub."Time To" then begin
                TimeCheckFrom := false;
                exit(TimeCheckFrom);
            end;
        TimeCheckFrom := true;
    ...
    begin
        if "Time From" <> 0T then
            if "Time From" > "Time To" then begin
                CheckTimeTo := false;
                exit(CheckTimeTo);
            end;
        CheckTimeTo := true;
