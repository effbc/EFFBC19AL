    ...
    fields
    {
        ...
        field(3;"Perk Code";Code[20])
        {
            ...
            begin
                ...
                if RVPERKS.Get(RvEmp."Job Title/Grade","Perk Code") then
                  begin
                    if PerkCodeFind then
                      Error(Text003,RVPERKS."Pay Perks Code",PayEmployeePerks."Employee No.");
        ...
        field(50030;Amount;Decimal)
        {
            ...
            begin
                ...
                if ("Perk Allot" = "Perk Allot"::Allocation) and ("To Date" = 0D) then
                  AsOnDate := WorkDate
                else if ("Perk Allot" = "Perk Allot"::Transfer) and ("To Date" = 0D) then
                  AsOnDate := "From Date"
                else
                  AsOnDate := "To Date";
            ...
            if PayEmployeePerks.Find('+') then
              begin
                if ("From Date" <> 0D) and ("From Date" = PayEmployeePerks."To Date") then begin
                  if ("From Date" < PayEmployeePerks."From Date") or ("From Date" < PayEmployeePerks."To Date") then
                    begin
                ...
                end else if (("From Date" <> 0D) and ("From Date" <= PayEmployeePerks."From Date")) or
                  (("From Date" <> 0D) and ("From Date" <= PayEmployeePerks."To Date")) then
                  begin
