table 33000912 "Issue Track Header"
{
    ...
    fields
    {
        ...
        field(2;"Issue Logged By";Text[50])
        {
            ...
            trigger OnLookup();
            begin
                  if UserId in ['EFFTRONICS\SUNDAR','EFFTRONICS\ANILKUMAR'] then
                  begin
                    UserGRec.Reset;
                    if PAGE.RunModal(9800,UserGRec)=ACTION::LookupOK then
                      "Issue Logged By":=UserGRec."User Name";
                  end;
            end;
        }
        ...
        {
            TableRelation = User.Field60100 WHERE ("User Name"=FIELD("Issue Logged By"));
        }
    }
    ...
    trigger OnDelete();
    begin
        if (UserId <> "Issue Logged By") then
        begin
